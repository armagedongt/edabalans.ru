...
Статус: `draft`
Состояние внедрения: проектирование до создания PostgreSQL-миграции и до импорта
персональных данных.
...
- `К_оплате` по уточнению владельца хранит реально внесённую сумму;
- `Тариф_Мастер_класса` смешивает предложения, маршруты бота и продуктовые
  названия и сам по себе не доказывает состав покупки.

Таблица является источником профилей messenger-пользователей, legacy-состояний,
атрибуции и фактической суммы `К_оплате`. Перед созданием финансовой операции
сумма обязательно сверяется с более надёжной лентой, чтобы не задвоить платёж.
...
| `К_оплате` | фактически оплаченная сумма; создавать фактический `payment`, если строка ещё не представлена более надёжной платёжной операцией |
| `Накопленные_Оплаты` | контрольный накопленный итог по человеку; не создавать отдельную оплату поверх `К_оплате` и других операций |
...
историческом `docs/APPLICATION_BACKLOG.md`; активными считаются только записи,
индексируемые через `docs/plans/README.md`.
