...
существенные правки владельца. Для нового текста `tools/prepare_author_post.py`
собирает пропорциональный задаче retrieval pack с маршрутом эмоции/аргумента,
эталонами, correction chains, компактной тематической историей и режимом
редактуры, а `tools/validate_author_draft.py` проверяет машинно проверяемые
границы. Нормативные режимы и проверочная политика принадлежат только
