...
У семьи в исходном серверном каталоге нет «главного оригинала»: Telegram, бот,
Pikabu и Telegraph являются равноправными неизменяемыми проявлениями. После
публикации собственной статьи её запись в `content/blog/manifest.json` становится
редакционным каноном семьи с ключом `blog:<slug>`. Прямые источники из
`source_provenance` и все члены уже подтверждённой семьи считаются проявлениями
этого канона; лишь предполагаемые совпадения требуют отдельного решения. Это не
переписывает исходный архив и не удаляет площадочные версии. Дальнейшее развитие
текста идёт из блога во внешние площадки; обратное обновление никогда не происходит
автоматически.

Для подготовки статьи блога поверх этой неизменяемой истории существует
отдельный канонический слой выбора основы:
`content/author-voice/source-selection/blog-source-families.json`. Он указывает
`preferred_basis`, предыдущие версии и дополнительные авторские источники до
публикации, а после публикации — правило перехода семьи под каноническую статью
блога. Проявления и их исходные версии не удаляются.
Человекочитаемая сводка решений находится рядом в
`blog-source-family-decisions.md`.
