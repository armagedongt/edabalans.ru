...
Изображения, видео и другие файлы collector **не скачивает**. В БД сохраняются только
исходный URL, тип, позиция и доступный preview URL. Для каждого материала обязательна
каноническая ссылка на оригинальную публикацию.
...
| `content_media` | только URL и метаданные изображений/видео, без бинарных файлов |
| `content_links` | wrapped и конечные URL, тип и исключение научных источников |
| `content_metric_snapshots` | изменяемые просмотры, рейтинг, сохранения и эмоции |
