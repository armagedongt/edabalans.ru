...
Тестовая и обычная отправки, включая retry, передают текст и кнопки через общий
`personalized_delivery` движка. Каждый получатель получает собственные ссылки;
шаблон draft не перезаписывается данными первого человека. Кнопка может содержать
целый поддерживаемый глобальный URL-шаблон, например `{{personal_masterclass_url}}`
или `{{personal_intensive_url}}`. `personal_intensive_current_day_url` требует
контекста конкретного шага интенсива и не используется как URL-кнопка разовой
рассылки; вместо него выбирают конкретный день. При отсутствующей CRM-привязке
или неразрешённом шаблоне отправка не происходит, получатель обычной рассылки
получает статус failed. Успешно отправленные получатели не пересылаются при retry.
...
flowchart TD
    ...
    P --> T{Нужен тест?}
    T -- да --> O[Подставить персональные ссылки и отправить одному owner-контакту]
    O --> P
    ...
    W --> S
    S --> PERSONAL[Подставить персональные ссылки отдельно для каждого контакта]
    PERSONAL --> R[Отправка и Sent / failed по каждому контакту]
    R --> F{Есть ошибки?}
