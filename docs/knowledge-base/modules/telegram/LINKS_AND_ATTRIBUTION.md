...
передаётся в одноимённую цель Метрики. Ссылка-
...
Статус: частично реализовано. Покупка уже связывается с `user_id` и последним
достоверным стартом с `yclid`; browser cookie и полный multi-touch путь остаются планом.
