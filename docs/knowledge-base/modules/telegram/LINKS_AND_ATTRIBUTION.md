...
browser cookie и полный multi-touch путь ещё не реализованы.
...
текущие метки страницы; browser cookie и полный multi-touch путь остаются
целевой схемой, а не условием работающего checkout.
...
Cookie на всём сайте не подключена. Передача источника в прямой checkout уже
...
сохраняет прежнюю привязку к достоверному старту; browser cookie и полный
multi-touch путь остаются целевой схемой.
...
cookie/browser visitor, полный multi-touch путь и детализация персонального
клика по конкретной отправке сообщения. Финальный статус offline-загрузки
