...
У каждой записи есть настройка `Показать страницу перед Telegram`.

- выключена: брендированный адрес мгновенно перенаправляет в Telegram;
- включена: открывается страница с кнопкой и подсказкой проверить VPN, затем
  пользователь сам открывает Telegram.

Настройка хранится в БД, а не кодируется шестым символом. Поэтому режим можно
изменить без замены уже опубликованной ссылки и без раздвоения статистики.

Ограничение: галочка действует только на наши `go.`-адреса. Прямая ссылка `t.me`
...
Переход на сайт нельзя узнать из Telegram автоматически. Ссылки из сообщений на
`mypohudenie.ru` должны проходить через отслеживаемый redirect или передавать
безопасный link token, а сайт — принимать событие. Связывание анонимного браузера с
человеком требует отдельного механизма и согласованных правил персональных данных.
