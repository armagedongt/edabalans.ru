...
Статус: `current`
Состояние внедрения: test-only; каноническая схема, одноразовое подключение,
событийные редакционные слоты, outbox-dispatcher и изолированный ускоренный режим
развёрнуты на тестовом Telegram-боте. Массовая отправка запрещена настройкой
`POSTPURCHASE_TEST_ONLY=true` до сквозного прогона владельца. Копии анкет,
явно запрошенные кнопкой участника, разрешены для всех пользователей отдельно
от автоматических рассылок (решение владельца от 17.09.2026).
...
До этой цепочки работает общий post-paid account onboarding. Первая подтверждённая
прямая покупка создаёт права и одно письмо с логином, паролем и ссылкой `/lk`.
Подключение мессенджера перенесено в обязательный материал первого дня политики
курса `v2`: Telegram и MAX равноправны, минимум один канал обязателен, второй
доброволен. Старые участники остаются на `v1`, где этот материал скрыт. Повторная
покупка на email с существующим паролем или повтор webhook не запускают новую
выдачу учётных данных и onboarding.

Это не одна длинная неразличимая рассылка, а две связанные цепочки:

1. **Начало курса** — обязательное подключение мессенджера внутри первого дня.
2. **Сопровождение во время мастер-класса** — DQS, две рецептурные точки,
   условные напоминания и персональные допродажи.
После завершения программы отдельный `postmasterclass_nurture` остаётся пустым
отключённым каркасом. Скрытых сообщений через 2, 4 и 7 дней после саморевью нет.
...
покупки, затем связать клиента с выбранным основным мессенджером в первом дне,
сопровождать его в контрольных точках мастер-класса и не предлагать уже купленное.
...
- `app_revealed_dqs` после первого допустимого открытия в четвёртом дне;
- `dqs_app_link_requested` только для осознанной повторной отправки из ЛК;
- первое открытие DQS;
...
- ожидание подключения канала без потери события;
...
1. данные клиента после подключения;
...
5. одна копия итогового саморевью участнику после явной отправки формы;
6. финальное недельное предложение.

Этот редакционный набор приводится к новой программе по дням и становится единым
источником пользовательских сообщений во время программы.
...
В админке показываются клиентские окна и служебные состояния. Над
...
| `PP_CLOSING_REVIEW_COPY` | клиент | участник отправил итоговое саморевью | нет связанного Telegram или активного доступа к Мастер-классу |
...
установлен `POSTPURCHASE_TEST_ONLY=true`, автоматические сообщения получают только
участники с включённым тестовым профилем. Исключение — явно запрошенные кнопкой
копии анкет: `messenger_identity`, `messenger_questionnaire`, `closing_review_copy`
и `current_diet_questionnaire`. Они доставляются всем участникам в их подключённый
мессенджер с обычной проверкой контакта, доступа и разрешения отправки.
Остальные накопленные напоминания и предложения этим исключением не запускаются. URL общей
...
Первое `closing_review_opened` отмечает только активность. Оно не ставит сообщения.
После `closing_review_submitted` создаётся одна идемпотентная outbox-запись с
полной копией анкеты для самого участника. Сергею автоматическое Telegram-
уведомление не создаётся. Подробный сценарий зафиксирован в
`docs/plans/MASTERCLASS_CLOSING_REVIEW_DELIVERY_SPEC.md`.

## Ускоренный тестовый режим

Ускоренный режим остаётся обратимой служебной возможностью тестового контура, а не
разделом пользовательской админки. Он сокращает только серверные таймеры дней и
due-сигналов выбранного тестового клиента, не меняет цены, покупки, права и
расписание остальных участников. Его включение, сброс прогресса и просмотр
технического состояния выполняются отдельной сервисной задачей. Production-рассылки
другим клиентам при таком тесте остаются выключенными.
...
подтверждена покупка мастер-класса
  ...
  → при первой прямой покупке выдать пароль по email и открыть ЛК
  → пройти обязательный material первого дня и подключить минимум Telegram или MAX
  → заполнена начальная анкета
  → по кнопке отправить в подключённый мессенджер данные участника и полную анкету
  → следом попросить переслать полученные сообщения Сергею
...
flowchart TD
  ...
  B --> C[Ждать начальной анкеты]
  C --> D[Сохранять каждый ответ автоматически]
  D --> E[Нажата кнопка Отправить в мессенджер]
  E --> F{Telegram или MAX уже подключён к ЛК?}
  F -- нет --> X1[Не отправлять персональные данные; показать ошибку]
  F -- да --> H[Выбрать подключённый канал]
  H --> I[Прислать данные клиента и полную анкету]
  I --> I0[Попросить переслать сообщения Сергею]

  I0 --> I1[День 1: временное предложение без публичного срока]
...
|---|---|---|---|
| `onboarding_questionnaire_completed` | Сохранить анкету; при подтверждённой связи создать две идемпотентные доставки строго в выбранный messenger account | Подтверждение отправки; без связи — понятная ошибка без утечки в старый контакт | Сначала прислать базовые данные и полную анкету, затем существующую просьбу переслать их Сергею; новую ссылку подключения не создавать |
| `current_diet_questionnaire_completed` | Сохранить ответы по 16 продуктовым категориям и создать одну задачу отправки | Подтверждение отправки | Прислать заполненный список в подключённый Telegram; если связи ещё нет — ждать её без дублей |
...
| `closing_review_submitted` | Сохранить ответы; не менять `started_at` окна | Полная копия анкеты самому участнику | Не ветвить по консультации и не уведомлять Сергея автоматически |
...
Граница слоя заканчивается вместе с текущей цепочкой мастер-класса. Отдельный
`postmasterclass_nurture` остаётся отключённым.

### Два источника входов

1. **Фактические доменные события** создаются действиями человека: покупка,
   анкеты, подключение Telegram, открытие дня, завершение материала, сохранение DQS,
...
- какой Telegram/MAX подключён и какой канал выбран основным;
...
- состояние начальной анкеты, опроса по продуктовым категориям, DQS и саморевью;
...
|---|---|---|---|
| `masterclass_purchase_confirmed` | Подтверждена оплата и есть доступ | `PP_WELCOME_PURCHASED`: коротко подтвердить покупку, дать кнопку страницы Мастер-класса; прежнюю продающую цепочку остановить | Ждать анкету/подключение; повтор покупки не создаёт второе welcome |
| `onboarding_questionnaire_completed` | Анкета отправлена, доступ к мастер-классу действует | Поставить `tpl_postpurchase_identity`, затем `tpl_postpurchase_questionnaire` в канал, уже подключённый к личному кабинету | Ответы читать из профильной таблицы при доставке; в событие их не копировать; повторное нажатие не создаёт дублей |
| `current_diet_questionnaire_completed` | Опрос дня 2 отправлен | Создать одну клиентскую задачу `PP_CURRENT_DIET`; шаблон `tpl_postpurchase_current_diet` подставляет `{{current_diet_formatted}}` из профильной таблицы | При отправке заново разрешить текущий preferred Telegram/MAX; повторная форма не создаёт вторую задачу; ответы не копировать в событие |
| `app_revealed_dqs` | Участник впервые допустимо открыл DQS в шаге дня 4 и доступ действует | Один раз поставить `tpl_postpurchase_dqs_app_link` в текущий preferred Telegram/MAX с обычной ссылкой и кнопкой на `https://edabalans.ru/dqs` | Reveal не откатывается при ошибке доставки; повтор reveal не создаёт дубль; ручной повтор возможен только из ЛК в явно названный канал |
...
| `closing_review_submitted` | Ответы сохранены | Поставить одну клиентскую доставку полной копии саморевью самому участнику | Текст не зависит от наличия консультации; Сергею автоматически ничего не отправлять |
...
| `masterclass_completed` | Завершено задание дня 21 | Остановить напоминания о прохождении курса | Отдельной цепочки 2/4/7 после саморевью нет |
...
- `support_notification_created` — служебное уведомление Сергею в сценариях, где
  оно отдельно утверждено; итоговое саморевью его не создаёт;
...
flowchart TD
  ...
  G -- первое раскрытие DQS или явный повтор из ЛК --> R[Поставить сообщение со ссылкой DQS]
  ...
  class C,E,H,I,J,L,M,N,R tech;
```

### Постоянная ссылка DQS

Первый допустимый reveal в `day-04-dqs` создаёт одну outbox-запись
`dqs_app_link` для текущего preferred Telegram или MAX.
Deduplication key строится от канонического reveal, поэтому повторное открытие
приложения не создаёт второго автоматического сообщения. Если ни один канал не
доступен, DQS всё равно открывается, а доставка фиксируется как недоступная.

Шаблон `tpl_postpurchase_dqs_app_link` сообщает, что DQS открыт, предлагает
закрепить сообщение и содержит обычную ссылку и обычную URL-кнопку «Открыть DQS»
на `https://edabalans.ru/dqs`. Telegram Web App и отдельная DQS-авторизация не
используются. При истёкшей браузерной сессии общий ЛК возвращает пользователя на
исходный адрес после входа.

В материале курса отдельной кнопки отправки нет. После reveal пользователь может
осознанно запросить новое сообщение из карточки DQS в ЛК отдельно кнопкой
«В Telegram» или «В MAX». Если выбранный канал ещё не связан, короткий
`named_delivery`-переход сначала связывает его, затем отправляет DQS-ссылку ровно
один раз. Это сервисная отправка, а не массовая цепочка; общий post-purchase граф
не включается.
...
В обязательном материале первого дня приложение запрашивает одноразовые
короткоживущие deep-link токены.
...
первый связанный канал становится основным, затем пользователь может явно выбрать
другой в личном кабинете.
...
- назначение `link_account` используется для материала, а `named_delivery` — для
  явной отправки в названный канал; `tg_tracking_sessions` не используется;
- генерация, Telegram-consumer и событие `messenger_link_confirmed` реализованы;
- успешное погашение сразу останавливает все presale-run CRM-пользователя;
  обычное подключение не запускает старые onboarding-сообщения `identity` и
  `questionnaire`, а named delivery выполняет только заявленную отправку;
...
Оплата использует единый first-party Robokassa checkout. Название, состав и цена
формируются только сервером из активной версии каталога и не принимаются из
браузера как источник истины. Прежние `ST100` и `#order:` — legacy, не текущий
маршрут.
...
`closing_review_submitted` сохраняет ответы в CRM и создаёт одну доставку полной
копии самому участнику. Анкета и сообщение одинаковы при любом тарифе. Бот не
пишет Сергею автоматически.

Кнопка «Получить саморевью в Telegram» завершает анкету и присылает участнику
вопросы с его ответами. Копию можно оставить себе или самостоятельно переслать
Сергею в личный диалог. Если участник хочет консультацию, он указывает удобный
формат и может заранее открыть ссылку «Как проходит консультация».

Открытие формы, автосохранение и наличие консультации не запускают отдельные
сообщения. Заготовки через 2, 4 и 7 дней после саморевью в эту цепочку не входят.
...
- каждый источник проактивных отправок обязан быть зарегистрирован и перед
  доставкой разрешать точную текущую основную messenger identity; скрытого
  fallback на другой канал нет;
...
2. Истёкшая или повторно использованная ссылка не связывает Telegram или MAX.
3. Конфликт двух содержательных CRM-клиентов уходит на ручную проверку.
4. Отсутствие доступного preferred Telegram/MAX явно подавляет отправку без
   скрытого fallback; история сохраняется.
...
9. Ошибка Telegram/MAX сохраняется в журнале и повторяется только до `expires_at`,
   затем доставка становится terminal `failed`.
...
- вычитать окончательные MAX-формулировки без изменения равноправной доставки;
