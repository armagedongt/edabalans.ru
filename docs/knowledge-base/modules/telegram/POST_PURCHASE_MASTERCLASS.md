...
Статус: `current`
Состояние внедрения: test-only; каноническая схема, одноразовая привязка,
...
Привязка мессенджера перенесена в обязательный материал первого дня политики
...
1. **Начало курса** — обязательная привязка мессенджера внутри первого дня.
...
и `current_diet_questionnaire`. Они доставляются всем участникам в их привязанный
...
подтверждена покупка мастер-класса
  ...
  → пройти обязательный material первого дня и привязать минимум Telegram или MAX
  → заполнена начальная анкета
  → по кнопке отправить в привязанный мессенджер данные участника и полную анкету
...
flowchart TD
  ...
  F -- да --> H[Выбрать привязанный канал]
...
|---|---|---|---|
| `onboarding_questionnaire_completed` | Сохранить анкету; при подтверждённой связи создать две идемпотентные доставки строго в выбранный messenger account | Подтверждение отправки; без связи — понятная ошибка без утечки в старый контакт | Сначала прислать базовые данные и полную анкету, затем существующую просьбу переслать их Сергею; новую ссылку привязки не создавать |
| `current_diet_questionnaire_completed` | Сохранить ответы по 16 продуктовым категориям и создать одну задачу отправки | Подтверждение отправки | Прислать заполненный список в привязанный Telegram; если связи ещё нет — ждать её без дублей |
...
1. **Фактические доменные события** создаются действиями человека: покупка,
   анкеты, привязка Telegram, открытие дня, завершение материала, сохранение DQS,
...
- успешное погашение сразу останавливает все presale-run CRM-пользователя;
  обычная привязка не запускает старые onboarding-сообщения `identity` и
