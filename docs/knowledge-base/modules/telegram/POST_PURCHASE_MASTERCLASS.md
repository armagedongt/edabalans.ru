...
| `dqs_app_link_requested` | Участник нажал «Отправить ссылку в Telegram», Telegram уже привязан и доступ к мастер-классу действует | Сразу поставить `tpl_postpurchase_dqs_app_link`: «Ваша система оценки качества питания — [открыть приложение]» | Это ручная сервисная отправка: не запускает MAX, не включает общую post-purchase рассылку и не создаёт автоматических повторов |
...
`https://похудение-это-есть.рф/dqs`.
