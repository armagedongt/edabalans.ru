...
До этой цепочки работает общий post-paid account onboarding. Подтверждённый
платёж Tilda или Robokassa создаёт один набор прав и письмо с двумя одноразовыми
ссылками выбора Telegram/MAX. Выбранный мессенджер привязывается к `user_id` и
выдаёт логин и пароль к собственному входу `/lk`. Скрытый шаг старого кабинета
«привязать мессенджер» законсервирован и для новых покупателей не показывается.
Повтор webhook или повтор уже погашенной ссылки не запускает вторую выдачу.
...
- `dqs_app_link_requested` после явной кнопки участника в материале DQS;
- первое открытие DQS;
...
подтверждена покупка мастер-класса
  ...
  → Telegram/MAX уже привязан при выдаче личного кабинета
...
| `current_diet_questionnaire_completed` | Опрос дня 2 отправлен | Создать одну клиентскую задачу `PP_CURRENT_DIET`; шаблон `tpl_postpurchase_current_diet` подставляет `{{current_diet_formatted}}` из профильной таблицы | Если Telegram ещё не связан — ждать; повторная отправка формы не создаёт вторую задачу; ответы не копировать в событие |
| `dqs_app_link_requested` | Участник нажал «Отправить ссылку в Telegram», Telegram уже привязан и доступ к мастер-классу действует | Сразу поставить `tpl_postpurchase_dqs_app_link`: прежняя ссылка остаётся в тексте, а кнопка «Открыть приложение» запускает тот же DQS как Telegram Web App | Это ручная сервисная отправка: не запускает MAX, не включает общую post-purchase рассылку и не создаёт автоматических повторов |
...
flowchart TD
  ...
  G -- явный запрос ссылки DQS --> R[Поставить одно сообщение со ссылкой DQS]
...
### Ручная отправка ссылки DQS

В материале DQS участник видит кнопку «Отправить ссылку в Telegram» только после
проверки привязанного Telegram. По нажатию сайт создаёт событие
`dqs_app_link_requested` и одну outbox-запись `dqs_app_link`; dispatcher ещё раз
проверяет активный `ACCESS_MASTERCLASS` и связанный Telegram-контакт. Шаблон
`tpl_postpurchase_dqs_app_link` содержит ровно текст: «Ваша система оценки
качества питания — открыть приложение», где последние два слова ведут на
`https://похудение-это-есть.рф/dqs`. Под сообщением находится кнопка «Открыть
приложение», которая открывает этот же HTTPS URL как Telegram Web App. Отдельная
копия DQS и отдельная Telegram-авторизация не создаются: приложение продолжает
использовать существующий вход Tilda.

Это не автоматическая цепочка и не рассылка: MAX не используется, новый контакт
не создаётся, а общий выключатель post-purchase сообщений не включается. Во время
режима обслуживания Telegram отправка остаётся ограничена allowlist. Если Telegram
не привязан, кнопка не показывается; если доступ отозван до доставки, задача
