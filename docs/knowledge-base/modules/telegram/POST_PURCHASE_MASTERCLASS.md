...
Статус: `implemented_disabled` — владелец утвердил каноническую схему; события,
due-очередь, приёмник и отключённый граф в админке приведены к одной модели.
Отправка намеренно выключена до утверждения текстов и отдельного production-запуска.
...
Этот существующий редакционный набор был подготовлен до новой программы по дням.
Его нельзя включать как есть: точки рецептов, напоминания и финальные окна должны
быть перестроены по схеме ниже.
