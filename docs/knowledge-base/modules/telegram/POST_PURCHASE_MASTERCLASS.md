...
Статус: `implementation_draft` — контур утверждён, редакционные блоки созданы,
реальная отправка ждёт подключения CRM-событий и production-бота
...
- связать редакционные блоки с реальными идемпотентными событиями CRM;
