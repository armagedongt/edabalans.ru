...
| `day_21_offer_opened` | Один раз зарегистрировать триггер `last_week`; ценовое окно остаётся серверным | Сообщить о последнем дне текущего окна либо показать уже начавшуюся последнюю неделю | Не подменяет отсчёт трёх сообщений: их якорь — первое `closing_review_opened` |
...
| `day_21_offer_opened` | Поставлен ценовой триггер последней недели | Обновить состояние предложения; сообщения недели после саморевью планируются отдельно от ценового таймера | Не создавать скрытый второй граф |
...
4. Создать версионированный событийный граф реакций курса и трёх сообщений дня 2/4/7 после саморевью.
