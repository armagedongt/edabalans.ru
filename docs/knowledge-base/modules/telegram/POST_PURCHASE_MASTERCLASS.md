...
| `app_revealed_dqs` | Участник впервые допустимо открыл DQS в шаге дня 4, Telegram уже привязан и доступ действует | Один раз поставить `tpl_postpurchase_dqs_app_link` с обычной ссылкой и кнопкой на `https://edabalans.ru/dqs` | Reveal не откатывается при ошибке доставки; повтор reveal не создаёт дубль; ручной повтор возможен только из ЛК |
...
`dqs_app_link`, если у пользователя уже есть активный связанный Telegram.
Deduplication key строится от канонического reveal, поэтому повторное открытие
приложения не создаёт второго автоматического сообщения. Если Telegram не связан,
DQS всё равно открывается.
...
осознанно запросить новое сообщение только из карточки DQS в ЛК через «Не нашли
ссылку? Отправить ещё раз». Это сервисная отправка, а не массовая цепочка: MAX не
используется, новый контакт не создаётся, общий post-purchase граф не включается.
