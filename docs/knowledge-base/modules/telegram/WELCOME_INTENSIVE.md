...
При каждом обычном `/start`, допущенном maintenance-политикой, бот отдельно от
контентного графа устанавливает для личного чата кнопку меню `Интенсив`. Кнопка
открывает те же web-страницы как Telegram Mini App по повторно используемой
персональной ссылке и восстанавливает текущую часть из PostgreSQL. Она не заменяет
callback `Начать интенсив`, не обходит проверку подписки и не меняет расписание
сообщений: это постоянная навигационная оболочка над уже существующим маршрутом.
...
8. Повторная кнопка снова запускает ту же проверку. Если подписка по-прежнему не
   найдена, отправить отдельный редактируемый `tpl_start_subscription_retry_reminder`:
   короткий текст со смайликом, ссылкой на канал и той же кнопкой. Первый
   пятиминутный срок при повторах не сдвигается.
...
flowchart TD
    ...
    SUB2 -- Да --> D1
    SUB2 -- Нет --> ASK2[Повторная просьба + «Проверить ещё раз»]
    ASK2 --> RETRY
    RETRY -- 5 минут --> D1
...
| персональная кнопка Mini App | `messenger_link_tokens(purpose=intensive_access)` + Telegram `setChatMenuButton` |
...
- при повторной неуспешной проверке отправляется отдельное редактируемое сообщение;
...
- повторный `/start` оставляет тот же рабочий персональный Mini App URL;
- повторное открытие URL на другом устройстве восстанавливает тот же прогресс;
