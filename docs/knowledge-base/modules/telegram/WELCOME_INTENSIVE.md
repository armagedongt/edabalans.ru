...
посещения сохранены, покупка мастер-класса проверена.
...
переход в основную рассылку. Здесь нет проверки покупки между постами: покупку
проверяет следующий модуль.

## 2. Последовательность

1. Отправить `tpl_start_navigation_pin` и попытаться закрепить его.
2. Отправить видеокружок `tpl_entry_circle`.
3. Отправить превью `tpl_start_welcome_offer`. В тексте нет ссылки на интенсив;
