...
2. один из согласованных входных текстов с персональной кнопкой «Открыть
   часть #1»: обычный `tpl_intensive_entry_default` либо вариант для
   Яндекс.Директа `tpl_intensive_entry_yandex`.

Персональная ссылка не содержит открытый Telegram ID. До выдачи всех четырёх
частей каждая кнопка ведёт прямо на соответствующую страницу
`/intensive/day-1` — `/intensive/day-4` с непрозрачным кодом, связывает открытие
с CRM-пользователем и не показывает сначала главную интенсива. После отправки
входа создаётся Welcome-run; День 1 считается выданным этим входом. Та же прямая
ссылка дня используется в его напоминании.

Повторный `/start` не перезапускает расписание:

- активный run → `tpl_intensive_entry_continue` с фактическим временем следующего
  сообщения и прямой ссылкой на последний выданный день;
- День 4 уже выдан → `tpl_intensive_entry_delivered` с одной ссылкой на главную
  интенсива, где доступны все части;
...
- `{{personal_intensive_day_1_url}}` — `{{personal_intensive_day_4_url}}`;
...
- вход, каждый день и каждое напоминание до завершения ведут прямо на страницу
  выданного дня; главная интенсива не используется как промежуточный экран;
- после доставки Дня 4 повторный Start и завершённый вход дают одну ссылку на
  главную интенсива;
