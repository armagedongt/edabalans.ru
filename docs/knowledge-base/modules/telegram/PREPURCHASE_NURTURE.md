...
Статус: `current`
Состояние внедрения: текущий граф и 17 редакционных окон находятся в production,
новая дедупликация самостоятельных материалов согласована и ожидает интеграции;
массовая доставка закрыта глобальным maintenance allowlist.
...
Каждый самостоятельный материал перед выдачей проверяется по существующему
каноническому тегу `Пост - Название`. Если человек уже получал материал, текущий
дневной слот не остаётся пустым: бот последовательно берёт следующий материал
очереди, которого у человека ещё нет. После успешной доставки назначается тег
фактически выданного материала. Варианты для подписанного и неподписанного имеют
разные `content_id`, но одно семейство и один тег.

Дата первого получения материала хранится в существующем
`user_tags.created_at` (runtime-модель `CrmUserTag`). Обычная очередь не повторяет материал независимо от
давности тега. Если Сергей решит вручную повторить старый пост, аудиторию можно
отобрать по дате назначения тега, например старше одного года. Первая дата не
затирается; новая отправка сохраняется в журнале доставок с новым `sent_at`.

Пример: материал «Маленькие шаги» стоит ориентировочно на 15-й день. Если человек
уже получил его как единственное напоминание во вступительной цепочке, в этот же
слот приходит следующий доступный материал. Несколько накопившихся сообщений
подряд не отправляются: один слот — один фактически выбранный материал.
...
| Канонические контентные теги и дата первого получения | `tags`, `user_tags.created_at`, `docs/TAG_RULES.md` |
...
7. Уже полученный материал заменяется следующим доступным в том же слоте, а не
   создаёт пустой день и не вызывает пачку сообщений.
8. Тег назначается только после успешной доставки фактически выбранного
   материала.

В админке: **Пользователи → Тест цепочки**. В окне выбирается Welcome либо
рассылка до покупки, ставится галочка ускорения и отдельно подтверждается очистка
только технического прогресса цепочек. Покупки, доступы, теги и CRM-карточка не
удаляются.
