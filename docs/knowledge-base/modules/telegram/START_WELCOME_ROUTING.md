...
Пока владелец заполняет тексты и вручную проверяет сценарии, перед атрибуцией и
любой цепочкой действует один глобальный maintenance allowlist:

1. Telegram contact и единый CRM user создаются или обновляются всегда.
2. Полная логика разрешена только Telegram ID из
   `TELEGRAM_MAINTENANCE_ALLOWED_USER_IDS`.
3. У остальных contact получает статус `maintenance_waitlist`, а в
   `tg_tracking_events` пишется `maintenance_contact`.
4. Пользователь получает редактируемый `tpl_maintenance_notice`; ни Start,
   кнопка, sequence, broadcast, ручное или post-purchase сообщение дальше не
   исполняются.
5. После окончания ремонта список можно выбрать по статусу
