...
7. Если мастер-класс куплен — отправить `tpl_start_has_masterclass` и завершить
   модуль.
...
До состояния `implemented/tested` ещё не готово:
...
- подробная развилка пока не является исполняемым графом; карта требований уже
  должна показываться отдельно от фактического исполнения;
- первый вход покупателя без заранее связанного Telegram account не умеет
  безопасно связаться с CRM по одноразовому токену;
- отдельный модуль Welcome ещё не собран в опубликованную версию;
- ветка `Welcome запускался, но run отсутствует и День 4 не отправлен` ещё не пишет
  отдельную структурированную ошибку для ручного разбора;
...
Следовательно, атрибуция внутри модуля работает, но весь глобальный модуль «Старт
и атрибуция» готов по требованиям и ещё не готов по исполнению.
...
## 12. Решения, которые нужны перед реализацией

1. Канонический признак `Welcome когда-либо запускался`: наличие любого
   `tg_sequence_runs` этого модуля плюс импортированный legacy-признак, если он
   существует. Нужно определить точный legacy-тег/поле; при его отсутствии человек
   без run считается не начинавшим Welcome.
2. Канонический факт покупки мастер-класса: активный доступ к продукту либо
   подтверждённая/оплаченная запись оплаты. Нужно утвердить этот вариант как общий
   для CRM и Telegram.
3. Если покупка обнаружена при активном Welcome/pre-purchase run, рекомендуется
   сразу завершать такой run, чтобы старые продающие сообщения не продолжались.
