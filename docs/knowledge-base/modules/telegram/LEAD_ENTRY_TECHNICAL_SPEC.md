...
Статус: `current`
Состояние внедрения: реализовано и развёрнуто на тестовом боте.
Версия: 1.0  
Дата: 22.08.2026  
...
- name;
- target kind;
- одно поле поиска existing tags с подсказками по мере ввода;
- добавление нескольких выбранных тегов в компактный список под полем и снятие
  выбранного тега до сохранения;
- явная кнопка `Создать новый тег` показывается только для введённого названия, для
  которого нет точного совпадения, и требует подтверждения;
