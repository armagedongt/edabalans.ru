...
- пока публичный сайт остаётся на Tilda, публиковать новые сайты и приложения
  только путями единого origin `edabalans.ru`, не создавая для них `app.`,
  `api.` и другие поддомены;
- после полного переноса публичного сайта на сервер публиковать новые сайты и
  приложения только путями `похудение.рф`;
- маршрутизировать уже работающие совместимые app/api/go/blog домены через Caddy
  до отдельной задачи на их перенос или вывод из эксплуатации; не переносить и
  не отключать их попутно при разработке нового маршрута;
...
- публиковать readiness-маршруты Telegram и MAX для внешнего мониторинга;
...
- сохранять старый вход бота `app.edabalans.ru/docs` (с завершающим `/` и без него)
  переходом на `https://похудение-это-есть.рф/legal`, сохраняя query-параметры;
...
Исходящий DNS и правила мониторинга/восстановления принадлежат `operations.health`:
