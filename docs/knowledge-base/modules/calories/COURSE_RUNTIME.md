...
Статус: `current`
Состояние реализации: трёхмодульная редакция реализована и прошла локальную приёмку.
Готовность конкретной публикации определяется активным manifest и наличием всех
статей через `publication_status`, а не состоянием seed в Git.
...
её выпуск 25.09.2026 вместе с проверенным курсом. Обновление, откат и повторное
обновление проверены на отдельной PostgreSQL-базе, восстановленной из свежей
production-копии; количество строк сохранено. Резервные копии обеих баз от
25.09.2026 19:39 UTC восстановлены штатным инструментом.
