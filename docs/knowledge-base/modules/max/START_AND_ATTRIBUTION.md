...
На границе MAX-доставки адаптер заменяет ссылки общего текста на MAX-эквиваленты:
Telegram-бот — на текущий MAX-бот с тем же payload, Telegram-канал и ссылки на его
посты — на канонический MAX-канал, личный Telegram username Сергея — на его
профиль в MAX. Поэтому в сообщениях MAX не остаются `t.me`, `telegram.me` и
`@FitnessSergey`; точные адреса задаются `MAX_CHANNEL_URL` и `MAX_CONTACT_URL`.
...
13. Для `apps`, `dqs`, `training`, `metabolism`, `recipes` не запускать Welcome и
    не назначать рекламные теги. Найти связанного пользователя, пересчитать права
    и события `app_revealed_*`, затем отправить только доступные кнопки `open_app`.
    MAX Mini App передаёт payload и подписанный `initData` на `/max-app`; backend
    проверяет подпись, возраст данных, MAX ID и право на ресурс.
