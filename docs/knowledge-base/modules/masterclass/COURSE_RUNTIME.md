...
4. Анкета завершается только после явной отправки формы; фоновое сохранение
   отдельного ответа оставляет попытку черновиком. Для любого шага типа
   `questionnaire` или `closing-review` сервер одной транзакцией сохраняет статус
   анкеты и завершение соответствующего открытого шага курса. Повторная отправка
   идемпотентна; прямое заполнение анкеты вне открытого места программы сохраняет
   ответы, но не обходит порядок обязательных материалов. Frontend может сразу
   показать следующий материал, однако чек-лист дня ждёт окончания фонового
   сохранения. Если клиент и сервер всё же разошлись, клиент обновляет прогресс,
   возвращает фокус к реально незавершённому материалу и не показывает участнику
   внутренний код вроде `materials_not_completed`. Шаг DQS завершается после
   ...
   после отправки. Рецепты и выделенный оффер завершают шаг после открытия
   отдельного экрана.
