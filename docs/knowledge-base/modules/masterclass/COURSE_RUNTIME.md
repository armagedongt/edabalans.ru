...
ручной ввод email ни для одного клиентского приложения: он получает email из
закрытой страницы Members Area, а при отсутствии профиля автоматически переводит
человека на штатный вход Tilda `/members/login`. Это сознательно менее сильная
граница идентификации, чем серверная сессия; при будущем уходе с Tilda она
заменяется, не меняя `user_id`, покупки и прогресс.
...
Одна физическая страница Tilda также является точкой входа для прямых ссылок:
