...
2. Внутри дня обязательные пункты проходят сверху вниз. Пункт с
   ...
   интерфейса для телефона или компьютера на отдельном полноэкранном слое и не
   является текстовой статьёй. В пяти экранах он также объясняет статусы
   материалов и автоматическое сохранение прогресса; отдельного экрана контактов
   в нём нет.
