...
Главный вход по видеоплеерам, их настройкам и размещениям —
[видео-контур](../../../../backend/app/static/video-player-development/README.md).
Здесь остаются правила прохождения МК и подключения медиа курса.
...
4. Анкета завершается после одного из двух явных действий: отправить заполненную
   форму в подключённый мессенджер или подтвердить, что голосовой ответ останется
   в дневнике. Фоновое сохранение отдельного ответа оставляет попытку черновиком.
   Для любого шага типа `questionnaire` или `closing-review` сервер одной
   транзакцией сохраняет статус анкеты и завершение соответствующего открытого
   шага курса. Повторное действие идемпотентно; прямое заполнение анкеты вне
   открытого места программы сохраняет ответы, но не обходит порядок обязательных
   материалов. После успешного выбора frontend активирует отдельную кнопку
   «Продолжить» и не перелистывает материал автоматически. Если клиент и сервер
   разошлись, клиент обновляет прогресс, возвращает фокус к реально незавершённому
   материалу и не показывает участнику внутренний код вроде
   `materials_not_completed`. При первом допустимом открытии DQS постоянная ссылка
   один раз ставится в очередь выбранного Telegram/MAX. Сам шаг DQS завершается
   после прохождения встроенного туториала до последнего экрана; заполнять строки
   для завершения материала не требуется. Приложение остаётся открытым: участник
   сам возвращается в день 4 и видит открытое задание. Рецепты и выделенный оффер
   завершают шаг после открытия отдельного экрана.
...
6. Галочки сохраняются на сервере по отдельности. День завершён, когда чек-лист
   открыт и отмечен хотя бы один видимый пункт. Остальные отметки необязательны
   и сохраняются как дополнительный прогресс. Расписание открытия следующего
   дня не меняется.
...
11. Состав курса зависит от закреплённой за пользователем версии правил. У старых
    участников `v1` прежний шаг «Подключить мессенджер» остаётся скрытым. Новая
    прямая покупка получает `v2`: этот шаг видим, обязателен и расположен в первом
    дне после материала о дневнике. Его завершение подтверждается сервером после
    связи хотя бы одного Telegram/MAX-аккаунта; второй канал можно подключить
    добровольно. Изменение активной программы не переводит старого участника на
    другую версию правил.
...
13. День с `accessResource` отдаёт обычные статьи только владельцу этого доступа.
    Остальным manifest показывает одну карточку экрана приобретения, прямые API
    материалов и исходных файлов не отдают закрытый текст, а открытие карточки
    запускает тот же серверный модуль предложения. Для дней 7–8 используется
    `ACCESS_RECIPES` и единый placement `recipes-part-1-gate`.
...
- `masterclass_day_progress`: первое открытие, IANA-часовой пояс дня, открытие
  ...
  завершение дня;
- `masterclass_step_progress`: завершённые обязательные пункты по стабильному
  `step_id`; индекс хранится как снимок позиции для диагностики, но не является
  идентичностью материала;
...
Учебное видео текущей программы предусмотрено только в шапке дня, не в отдельном
материале. Старый `materialMedia` — технический резерв, не поддерживаемый формат
нового учебного материала. Если такой формат будет отдельно введён, он должен
использовать тот же `standard` с обложкой и главами: передачу данных материала
потребуется реализовать, сейчас она неполна. Стороннее видео-пример по месту
в статье не становится учебным видео: правила выбора и встраивания принадлежат
[единому видео-входу](../../../../backend/app/static/video-player-development/README.md).
...
как видимый/обязательный шаг клиенту. Добавление, удаление и перестановка
материалов не меняют идентичность уже пройденного шага: runtime сопоставляет
прогресс по `step_id`. Удаление или переименование самого `step_id` остаётся
изменением данных и требует явной миграции.

Вложенная статья (`nested: true`, `required: false`, `parentStepId`) не является
скрытой: runtime и редакторский API находят её по стабильному `step.id`. При этом
frontend исключает её из оглавления, линейной навигации, времени дня и прогресса.
Доступ проверяется по курсу и дневной границе так же, как у родителя; без
`ACCESS_RECIPES` вложенные рецепты не выдаются.

## API

- `GET /api/masterclass/course/manifest` — текущая версия, 20 дней и
  исполняемый порядок шагов;
- `GET /api/masterclass/course` — программа доступности и прогресс;
- `GET /api/masterclass/course/content/{asset_name}` — разрешённый
  исходный текстовый fallback из whitelist манифеста;
- `GET /api/masterclass/course/materials` — опубликованные в БД версии
  обычных и вложенных статей уже открытых или доступных к открытию дней по `step.id`;
...
выбирает человека и не выдаёт право. Прямая покупка может один раз создать такую
сессию через связанный с браузером 60-минутный grant; затем действует обычная
45-дневная сессия личного кабинета.
...
Названия, порядок, вводный текст дней и задания берутся из единого `program.md`;
старые `days/*.md` остаются архивом. Подводки материалов берутся из их MD.
Определения вопросов, подсказки и подписи двух кнопок анкет берутся из MD
материалов и сохраняются в версии структуры. Кнопка отправки сохраняет ответы и
ставит сообщение в очередь, кнопка голосового фиксирует выбор без отправки пустой
анкеты. После любого успешного выбора активируется отдельная кнопка «Продолжить»;
автоматического перехода к следующему материалу нет.
Ответы остаются привязаны к прежним кодам вопросов. Самостоятельные страницы,
встроенные анкеты, CRM и сообщения используют опубликованные определения;
публикация сама по себе не отправляет участникам сообщения.
Для выпуска только первых пяти дней используется `--through-day 5`:
остальные дни и их материалы не публикуются заново. Завершающее упражнение
дня 3 — отдельная статья `day-03-practice`, добавленная из авторской оболочки дня.
...
```mermaid
flowchart TD
    A[Вход: app_session + ACCESS_MASTERCLASS] --> B[Открыть доступный день]
    ...
    N --> O[Выход: текущий preferred Telegram/MAX]
```

## Ошибочные состояния

- нет действующей `app_session` — приложение сохраняет исходный URL и
  перенаправляет на `/lk?next=...`; после входа возвращает в тот же материал;
