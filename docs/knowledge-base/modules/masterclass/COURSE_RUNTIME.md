...
временное окно. При первом просмотре без ранее запущенной ступени `day-1-offer`
возвращает `expires_at = null`, а интерфейс не рисует ни срок, ни таймер. Если
та же ступень уже была запущена реальным серверным триггером, повторное открытие
показывает её сохранённый срок и не создаёт новый отсчёт.
