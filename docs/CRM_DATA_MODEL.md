...
ссылкой. В ней хранятся выбранные ресурсы, снимок обычной и итоговой цены, режимы
открытия, срок, статус и связанный checkout. В базе хранится только SHA-256 hash
случайного токена; email, цена и права не помещаются в URL.
...
Отдельно от права хранит `paced` или `fully_unlocked` для пары пользователь и
ресурс. Поэтому бесплатное открытие всех дней не создаёт новый продукт и не
подменяет запись в `user_accesses`.
