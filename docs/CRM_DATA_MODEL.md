...
flowchart LR
  ...
  UT --> T[tags\nсправочник тегов]
  U --> N[client_notes\nзаметки]
  U --> LA[user_legal_acceptances\nверсии согласий]
  U --> MDP[masterclass_day_progress\nдни и задания]
  U --> MSP[masterclass_step_progress\nматериалы по порядку]
...
### `messenger_link_tokens` — одноразовая привязка аккаунта

Короткоживущая техническая таблица для безопасного перехода из анкеты
мастер-класса в Telegram. Она не является вторым справочником клиентов и не хранит
сам открытый start-token.

- Связь: `messenger_link_tokens.user_id -> users.id`.
- Хранится только SHA-256 hash токена, платформа, назначение `link_account`, срок
  15 минут, время создания и будущего погашения.
- В Telegram URL нет email и `user_id`; в БД нет открытого токена. Таблица хранит
  обязательный внутренний `user_id` как FK и только SHA-256 hash start-токена.
- Повторная генерация инвалидирует предыдущий активный токен того же человека.
- После погашения Telegram-модуль связывает аккаунт через
  `messenger_accounts`; сама таблица токенов не дублирует эту постоянную связь.
- Конфликт с Telegram-аккаунтом другого содержательного клиента отправляется на
  ручную проверку и не перезаписывается автоматически.
...
Для серверных checkout дополнительно сохраняются `pricing_version_id` и
`price_entry_code`. Это неизменяемый снимок правила цены, действовавшего в момент
заказа. Изменение текущего прайса не меняет историю и не переопределяет тариф
старой покупки.

`amount_is_estimated=false` означает известную фактическую сумму. Значение
`amount_is_estimated=true` разрешено только для явно обозначенной реконструкции по
старому тарифу/доступу. Такие суммы показываются с `≈`, считаются отдельно и не
входят в подтверждённую выручку. Накопленная сумма клиента в карточке складывается
из всех его фактических операций; оценочная часть показывается рядом отдельно.

Для LeadTeh действует согласованное правило: заполненное `К_оплате` означает
реально оплаченную сумму. `Накопленные_Оплаты` используется как контрольный итог и
не создаёт вторую операцию поверх уже учтённых платежей.
...
Для Мастер-класса стабильный `products.code` определяет пользовательское название
тарифа: `MASTERCLASS_BASIC` — «Минимальный», `MASTERCLASS_RECIPES` —
«Стандартный», `MASTERCLASS_CONSULT` — «С консультацией». Личный кабинет,
CRM-карточка и Telegram-сводка читают нормализованный продукт подтверждённого
платежа. Журнал `payments` и действующие права `user_accesses` остаются разными
сущностями: покупка отвечает на вопрос «что и по какому тарифу купили», доступ —
«что сейчас разрешено открыть». Новая таблица-дубль для карточки клиента не
создаётся.
...
- Приватные ручные ведомости импортируются через
  `backend/app/importers/manual_payments.py`: сначала обязательный dry-run, затем
  применение после backup. Исходные TSV/XLSX и персональные данные в Git не попадают.
...
### `masterclass_day_progress` — состояние дня Мастер-класса

Одна строка хранит состояние одного дня одного участника: первое открытие,
открытие задания, отмеченные вручную пункты и момент завершения дня.

- Связь: `masterclass_day_progress.user_id -> users.id`.
- Пара `user_id + day_number` уникальна.
- Сервер, а не браузер, определяет момент открытия следующего дня.
- Галочки хранятся в JSON только как состояние фиксированного задания этого дня;
  ключами служат стабильные ID пунктов, а тексты остаются в активной структуре.
- `structure_revision_no`, `required_step_ids` и `required_check_ids` фиксируют
  обязательный набор на момент первого открытия дня. Скрытые элементы исключаются
  из обязательности, а завершённый день редакционной правкой не отменяется.

### `masterclass_step_progress` — прохождение материалов по порядку

Одна строка означает, что участник завершил конкретный обязательный шаг дня:
статью, анкету, приложение, привязку мессенджера или экран предложения.

- Связь: `masterclass_step_progress.user_id -> users.id`.
- Тройка `user_id + day_number + step_index` уникальна.
- Следующий шаг сервер принимает только после предыдущего; прямой URL или изменение
  браузерного состояния не позволяют перескочить обязательный материал.
- Само открытие статьи не считается чтением: завершение фиксирует отдельная кнопка.

### `managed_document_versions` — версии управляемых документов

Общий минимальный механизм хранит неизменяемые JSON-снимки редакционных документов:
тип, стабильный ключ, версию схемы, номер ревизии, hash, автора, время и признак
активности. Для одной пары `document_type + document_key` активна ровно одна версия.

Первый документ — `course-structure/masterclass-21`. Клиентский manifest и backend-
проверки читают одну активную ревизию. Исходный `course.json` нужен только для seed,
если документ ещё не создан. Хранятся последние 20 ревизий; возврат создаёт новую
активную ревизию и не переписывает историю.

## 4. Продукты и доступы

### `pricing_versions` — версии единого прайса

Хранит черновую, активную и архивные версии цен. Активная версия после публикации
не редактируется; следующее изменение создаётся копированием в новый черновик.

### `price_entries` — строки цены конкретной версии

Одна таблица содержит три тарифа сайта, базовые цены продуктов и ступени
допродаж. Строка хранит стабильный код, состав ресурсов, обычную, зачёркнутую и
фактическую цену, активность и порядок. Цена сама не является доказательством
тарифа: checkout связывается с точным кодом строки и версией, а сумму сервер
...
### `personal_access_links` — персональные решения и предложения владельца

Одна запись связывает конкретного человека с бесплатной или платной персональной
ссылкой. В ней хранятся выбранные ресурсы, снимок обычной и итоговой цены, для
каждого ресурса — режим старта и режим открытия материалов, срок, статус и
связанный checkout. В базе хранится только SHA-256 hash случайного токена; email,
цена и права не помещаются в URL.

### `offer_checkouts` — короткоживущий снимок заказа

Хранит точный состав ресурсов, сумму, срок, тип checkout, а для нового сайта и
каталожных допродаж — `pricing_version_id` и `price_entry_code`. Публичный checkout
может быть создан до определения пользователя; email из подтверждённого webhook
Tilda связывает его с клиентом. Сервер не выдаёт права по одной лишь сумме.

### `user_legal_acceptances` — история подтверждённых документов

Одна строка фиксирует подтверждение конкретным пользователем конкретной версии
дисклеймера или политики обработки персональных данных. Хранятся код документа,
неизменяемая версия, источник и время принятия.

- Связь: `user_legal_acceptances.user_id -> users.id`.
- Тройка `user_id + document_code + document_version` уникальна.
- Новая редакция создаёт новую версию и не переписывает старое доказательство.
- Таблица не является правом на продукт: покупка и доступ остаются в своих таблицах.
- До перехода на собственную авторизацию источник `tilda_members_area` означает,
  что пользователь определён по email закрытой страницы Tilda; это ограничение
  доказательной силы зафиксировано отдельно в базе знаний.

### `user_course_policies` — режим прохождения уже доступного курса

Отдельно от права хранит `start_mode` (`open` или `blocked`) и `unlock_mode`
(`paced` или `fully_unlocked`) для пары пользователь и ресурс. Поэтому бесплатное
открытие всех дней не создаёт новый продукт и не подменяет запись в
`user_accesses`. В карточке CRM это показывается тремя независимыми ступенями:
право на курс, возможность начать и открытие всех уроков. Отсутствие права всегда
оставляет две последующие ступени закрытыми.

### `account_onboardings` — устойчивая очередь выдачи учётных данных

Одна строка описывает одно отложенное письмо. Обычная самостоятельная регистрация
и post-paid onboarding используют одноразовый выбор Telegram/MAX. Ручное создание
человека в CRM использует тот же SMTP-worker, но с `delivery_mode=direct_password`:
в письме находятся готовые логин, пароль и адрес входа. Содержимое очереди
зашифровано ключом приложения; неотправленное письмо со старым паролем помечается
как устаревшее после нового выпуска пароля.
