...
2. производная карта модулей пересобирается из целевого commit для tests/build;
3. проверяется Docker Compose;
4. собирается backend и запускаются автоматические тесты;
5. VM видит успешный тест через публичный GitHub API;
6. сервер получает конкретный проверенный Git-коммит и так же пересобирает карту;
