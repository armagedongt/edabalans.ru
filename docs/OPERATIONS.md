...
Классификатор выпуска читает Git-пути с `core.quotepath=false`: изменения
учебных Markdown с русскими именами запускают те же проверки и сборку backend,
что и файлы с латинскими именами. Это покрыто проверкой реального Git diff.
...
Канон DNS и его отката перенесён в модуль
[`operations.health`](runtime-health/README.md#исходящий-dns-всего-российского-сервера).
Этот заголовок сохранён для старых ссылок; настройки здесь не дублируются.
...
Доступность сервера, Telegram и MAX, уведомления, остановка/возврат рекламы,
локальные перезапуски, DNS, проверки и откат описаны в едином
[`docs/runtime-health/README.md`](runtime-health/README.md).
Внешний watchdog выпускается отдельно от серверного deploy; порядок указан там.
