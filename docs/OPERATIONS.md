...
На 05.09.2026 автоматическое восстановление Timeweb включено. Токен Яндекс.Директа
уже хранится в Cloudflare secrets, но остановка рекламы ещё не активна: отдельный
рабочий поток «Я.Директ!» обязан подтвердить точные `YANDEX_CAMPAIGN_IDS`, определить
необходимость `YANDEX_DIRECT_CLIENT_LOGIN` и проверить кампании через `Campaigns.get`.
До этого Worker не вызывает `Campaigns.suspend` и сообщает о неполной конфигурации,
если авария достигнет десятиминутного порога.
