...
`pnpm test`, `pnpm check`, `pnpm deploy`. После первого выпуска отдельно записывается
secret `RELAY_TOKEN`; при обычном обновлении он сохраняется Cloudflare. Адрес Worker
содержит случайный суффикс и не публикуется в документации, чтобы его нельзя было
угадать и забить бесплатную квоту посторонними запросами. Откат выполняется теми же
