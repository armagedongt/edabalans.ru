...
`SMTP_STARTTLS`. Для Яндекс Почты использовать отдельный пароль приложения. После
перезапуска backend выполнить реальный вход тестовой почтой и проверить, что код
приходит, действует 10 минут, а повторная отправка ограничена одной минутой.
