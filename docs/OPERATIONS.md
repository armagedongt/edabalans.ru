...
Production-проверка после выпуска 04.09.2026 сообщает `telegram_route=proxy`, поэтому
свежий успешный polling сейчас одновременно подтверждает и европейский шлюз.
...
- общая `/ready` доступна, а Telegram readiness нет — один раз перезагружается
  европейская VM, через пять минут сохраняющейся ошибки — российская VM;
- через десять минут от первой ошибки останавливаются только перечисленные в runtime
  ID кампаний методом `Campaigns.suspend`;
- после восстановления приходит сообщение, но реклама автоматически не возобновляется.

На 06.09.2026 автоматическое восстановление Timeweb включено. Токен Яндекс.Директа
...
десяти минут Worker может остановить только эти две кампании; автоматически
...
ошибку европейского proxy перезапуском контейнера.
...
через него. Старый нерабочий HTTP-proxy 27.08.2026 вызывал `ConnectTimeout`, но эта
запись больше не описывает текущий работающий маршрут.
