...
## Исходящий DNS российской VM

Владелец сетевой настройки — `operations.proxy`, владельцем мониторинга и
автоматического восстановления остаётся `operations.delivery`. Этот документ —
общий операционный канон для всех чатов. Чат, меняющий production-поведение,
обновляет его и карточку затронутого модуля; память отдельного чата не заменяет канон.

Российская VM использует два независимых не фильтрующих DNS-сервиса: Яндекс
`77.88.8.8` и Cloudflare `1.1.1.1`. Старые четыре DNS из DHCP/IPv6 RA Timeweb
не используются: они принадлежат одному провайдеру и не дают независимого резерва.
DHCP по-прежнему выдаёт IP и gateway; меняется только выбор DNS.

Источник настройки —
[`infra/network/10-netplan-eth0.network.d/90-edabalans-dns.conf`](../infra/network/10-netplan-eth0.network.d/90-edabalans-dns.conf).
Установленная копия —
`/etc/systemd/network/10-netplan-eth0.network.d/90-edabalans-dns.conf`.
Drop-in дополняет Netplan-generated `/run/systemd/network/10-netplan-eth0.network`,
не меняет cloud-init файл и сохраняется после reboot/перегенерации Netplan.
Если имя базового `.network` изменится, установщик должен перенести drop-in к
фактически используемому файлу из `networkctl status eth0`.

Путь запроса бота: Docker DNS `127.0.0.11` → host resolver `127.0.0.53`
(`systemd-resolved`) → текущий DNS из этих двух. Resolver переключается при ошибке,
а не опрашивает резерв непрерывно: первый запрос при отказе может ждать таймаут,
поэтому мгновенное бесшовное переключение не обещается. Это резерв поиска адресов,
не резерв интернета, VM, Cloudflare relay или Telegram. Полный сетевой обрыв
Timeweb DNS-настройкой не лечится. Публичные DNS-записи доменов, Tilda, атрибуция,
токены и сообщения бота не меняются.

Установка — отдельная ограниченная host-операция, не server/container deploy.
После проверки обоих DNS с VM установить source-файл с правами `0644` в указанный
drop-in каталог, выполнить `networkctl reload`, затем проверить `resolvectl status`:
у `eth0` должны остаться только эти два upstream. Убедиться, что DHCP IP/gateway
сохранены и в журнале `systemd-networkd` нет ошибок разбора. Проверить поиск адресов
GitHub и фактического relay внутри бота, `/ready` и `/api/health/telegram` с
`telegram_route=relay`. Перезагрузка VM/Docker для DNS-правки не нужна.
Один `FallbackDNS=` в `resolved.conf` не решает задачу: при полученных по DHCP
DNS он не является требуемым независимым резервом.

Откат к исходному DHCP DNS: удалить только указанный `90-edabalans-dns.conf`,
выполнить `networkctl reload`, `resolvectl revert eth0` и `resolvectl flush-caches`;
проверить возврат DNS Timeweb и обе readiness-точки. Базовый Netplan-файл не удалять.
При замене уже существующего drop-in сначала сохранить его вне репозитория и при
откате вернуть эту копию вместо удаления. База/миграции/рассылка не затрагиваются.
...
Если включён `ACCOUNT_ONBOARDING_ENABLED`, общая `/ready` дополнительно проверяет,
что заданы SMTP-доступ, адрес отправителя, ссылки обоих ботов, публичный адрес ЛК и
что worker писем не выключен. Неполная настройка возвращает `503`: новый сценарий
нельзя считать готовым к продажам, пока письмо с выдачей доступа заведомо некуда
отправлять.

`/robokassa-test` — временная `noindex`-страница с одной кнопкой для проверки
...
при `ROBOKASSA_LIVE_PROBE_ENABLED=true`, использует боевой пароль № 1 независимо
от глобального тестового режима и не создаёт аккаунт, продуктовый доступ,
onboarding или письмо. После одной диагностической оплаты флаг выключается в
`/opt/edabalans/.env`, затем выполняется `docker compose up -d --force-recreate backend`.
Tilda и общие URL магазина при этом не меняются.

Контрольная реальная оплата успешно подтверждена 06.09.2026; production-флаг
после неё установлен в `false`, а страница возвращена в состояние `404`.

Серверная подписка `/subscription` запускается только после миграции
`20260912_0041`, добавления непустого `ROBOKASSA_PASSWORD_2` в серверный `.env`
и установки `ROBOKASSA_RECURRING_WORKER_ENABLED=true`. `/ready` блокирует запуск
неполной конфигурации: для проверки отказа нужны оба боевых пароля, а для одного
уведомления о неудачном списании — рабочие SMTP-настройки. Значения секретов при
проверке не печатать. До включения worker страница и первая оплата не должны
публиковаться покупателям.
...
российский сервер и весь реально используемый путь от него до Telegram API.
Production использует отдельный Cloudflare Telegram relay и сообщает
`telegram_route=relay`. Свежий успешный polling подтверждает весь фактический путь:
российская VM → Cloudflare relay → Telegram API → бот.
Локальный `/health` остаётся простой проверкой процесса для Docker и не заменяет
внешний контроль. Остальные веб-приложения используют те же Caddy, backend и
PostgreSQL, поэтому отдельные проверки каждой страницы не создаются.

Cron запускается раз в минуту и делает две проверки с интервалом 30 секунд. После
трёх последовательных ошибок, примерно через минуту от первой, Cloudflare
открывает один incident и отправляет уведомление владельцу через `sendMessage`
отдельного `@TetrisgfgfgfBot`. Cloudflare
не читает updates и не конкурирует с production polling. Токен и chat ID находятся
только в Cloudflare secrets. Один incident не создаёт поток одинаковых сообщений.

Автоматическое восстановление:

- недоступна общая `/ready` дольше четырёх минут — один раз запрашивается reboot
  российской VM через Timeweb API; этот запас не даёт внешнему watchdog прервать
  штатный deploy, которому разрешён запуск до 180 секунд;
- общая `/ready` доступна, а Telegram readiness нет — серверы не перезагружаются:
  внешний маршрут reboot не чинит, а перезапуск только увеличивает простой;
- при открытии incident примерно через минуту от первой ошибки останавливаются
  только перечисленные в runtime ID кампаний методом `Campaigns.suspend`;
- только после двух полных минут и минимум четырёх последовательных успешных
  проверок
  восстановления, методом `Campaigns.resume` возобновляются только кампании,
  которые автоматика сама остановила в текущем incident. Кампании, остановленные
  вручную до аварии, не запускаются. Во время открытого incident нельзя вручную
  менять состояние перечисленных кампаний при включённом `ACTIONS_ENABLED`:
  Яндекс.Директ не сообщает, кем выполнена остановка, а потерянный ответ на
  `suspend` неотличим от применённой команды. Перед ручным вмешательством нужно
  выключить `ACTIONS_ENABLED`.

На 16.09.2026 автоматическое восстановление Timeweb оставлено только для полной
недоступности основной платформы. Токен Яндекс.Директа
хранится в Cloudflare secrets; `YANDEX_CAMPAIGN_IDS` содержит поиск `714152601` и
РСЯ `714157420`. Обе кампании проверены через `Campaigns.get` от собственного
аккаунта, поэтому `YANDEX_DIRECT_CLIENT_LOGIN` не требуется. При аварии дольше
минуты Worker может остановить только эти две кампании; при восстановлении он
может возобновить только фактически остановленные им ID после двух минут
непрерывной исправной работы и минимум четырёх положительных наблюдений. Любая
новая ошибка обнуляет интервал и счётчик; пропущенные cron-запуски успехами не
считаются.

Reboot каждой VM отправляется не более одного раза за incident: потерянный ответ
Timeweb нельзя надёжно отличить от уже принятой команды, поэтому повторный reboot
опаснее ручной проверки. Остановка рекламы после временной ошибки API повторяется
каждые две минуты до успеха. Список campaign ID валидируется целиком: одна опечатка
блокирует запрос и вызывает alert, а не оставляет часть рекламы незаметно включённой.

Известное ограничение защиты рекламы: если `Campaigns.resume` применился частично
или его ответ потерян, incident остаётся открытым. При новом отказе Telegram до
завершения этого восстановления уже включённые кампании могут не получить повторный
`suspend`: завершённое действие остановки сохранено в том же incident. На
16.09.2026 этот сценарий воспроизведён изолированно, но не исправлен в DNS-выпуске;
нужна отдельная согласованная правка восстановления. В таком incident нельзя
считать остановку всех кампаний гарантированной; правило отключения
`ACTIONS_ENABLED` перед ручным вмешательством остаётся обязательным.

Сильное incident-состояние хранит один SQLite-backed Durable Object, поэтому два
параллельных запуска не отправляют конкурирующие reboot-команды. Бесплатный тариф
допускает 100 000 Worker-запросов в сутки; cadence в 30 секунд выполняет 2 880
проверок в сутки. Вместе с обычным long polling relay базовый расход остаётся
существенно ниже бесплатного лимита.
Суточный маркетинговый отчёт исполняется в отдельном Durable Object и не стоит в
очереди перед 30-секундными health-проверками. Миграционный cutover фиксирует уже
отправленный отчёт за 08.09.2026 и одноразовые preview, чтобы новый объект не
дублировал их. Отказ первого запуска или отчёта не отменяет отложенную проверку.
Автоматические действия включаются только после записи runtime secrets, тестового
уведомления и установки `ACTIONS_ENABLED=true`.

Alert drill выполняется через закрытые `POST /drill/fail` и `/drill/recover` с
`DRILL_TOKEN`: три fail-вызова открывают тестовую аварию, четвёртый подтверждает
отсутствие дубля, пять recover-вызовов имитируют две полные минуты
устойчивого восстановления и
закрывают её. Drill использует отдельное Durable Object
state и кодом блокирует reboot/Direct, поэтому production-серверы и реклама не
затрагиваются. Ожидаются два Telegram-сообщения — авария и восстановление.
Публичный `workers.dev` URL в постоянной конфигурации выключен, чтобы внешний трафик
не мог исчерпать бесплатную квоту и заглушить cron. Он кратковременно включается
только при первичном authenticated drill и сразу отключается повторным deploy.

Timeweb secret обязан быть ограниченным токеном только с правом «Облачные серверы —
Управление»; доступ к остальным сервисам и удаление без Telegram-кода выключены.
Cloudflare Worker имеет отдельный release path из
`infra/monitoring/cloudflare-watchdog`: `pnpm install --frozen-lockfile`, тесты,
`pnpm deploy`. В журнал выпуска записываются Git SHA и Cloudflare account/subdomain,
секреты не записываются. Откат выполняется повторным `pnpm deploy` из последнего
исправного Git SHA.

Telegram relay выпускается независимо из
`infra/monitoring/cloudflare-telegram-relay`: `pnpm install --frozen-lockfile`,
`pnpm test`, `pnpm check`, затем `pnpm deploy` с обязательным непубличным
`PRODUCTION_RELAY_NAME`. После первого выпуска отдельно записывается
secret `RELAY_TOKEN`; при обычном обновлении он сохраняется Cloudflare. Адрес Worker
содержит случайный суффикс и не публикуется ни в документации, ни в Git, чтобы его
нельзя было угадать и забить бесплатную квоту посторонними запросами. Откат выполняется теми же
командами из последнего исправного Git SHA, затем readiness обязана снова показать
`telegram_route=relay`.

Relay и основной watchdog находятся в одном Cloudflare account. Это не влияло на
аварию 09.09.2026 и не возвращает нестабильную связь российской VM с европейской,
но остаётся общей границей для редкого account-wide сбоя Cloudflare. Не считать этот
риск закрытым отдельным процессом на российском сервере: для полной независимости
нужен второй внешний provider наблюдения.

Старая европейская VM со Squid может быть выключена после периода наблюдения: она
не участвует в текущем маршруте и не является автоматическим резервом. Точный
аварийный порядок возврата на неё, синхронного переключения watchdog и
воссоздания VM после удаления хранится только в
[`infra/telegram-proxy/README.md`](../infra/telegram-proxy/README.md).

На российской VM `edabalans-telegram-watchdog.timer` раз в минуту проверяет Docker
health только контейнера `telegram-bot`. Состояние `unhealthy` перезапускается не
чаще одного раза в пять минут; во время deploy проверка пропускается по общему lock.
Этот локальный слой чинит зависший процесс быстрее внешнего, но не пытается лечить
ошибку внешнего маршрута перезапуском контейнера.

Исходники, конфигурация и инструкция подключения находятся в
`infra/monitoring/cloudflare-watchdog/`. Секреты и реальные ID серверов/получателя
...
Для согласования будущей рассылки старым пользователям собирается read-only CSV:

```bash
docker compose exec backend python -m app.importers.tilda_access_migration_report \
  /tmp/tilda-access-migration.csv
```

Отчёт читает последний импортированный снимок Tilda, новые права, прогресс и
связанные Telegram/MAX ID. Он ничего не меняет в базе и не отправляет сообщения.
Файл содержит персональные данные, поэтому хранится только во временном каталоге
или закрытом хранилище и не попадает в Git. В колонке пароля указывается только
наличие уже созданной учётной записи: хеш нельзя и не нужно превращать в открытый
пароль.
...
9. проверяются общие health endpoints и временная публичная страница оплаты на
   go-домене;
...
Production использует `TELEGRAM_API_BASE_URL` и `TELEGRAM_GATEWAY_TOKEN` для
закрытого Cloudflare relay. `TELEGRAM_PROXY_URL` пуст. Европейская VM со Squid не
входит в рабочую цепочку Telegram: 09.09.2026 маршрут от российской VM к её IP
одновременно терял TCP-соединения на `22` и `3128`, хотя сама европейская VM и её
выход к Telegram оставались исправны. Relay устраняет эту сетевую границу.
...
## Выпущенный единый каталог цен

Migration `20260823_0020` добавила версионируемый каталог цен и поля снимка цены
в checkout/оплату. В production каталог включён:

```dotenv
PRICING_CATALOG_ENABLED=true
```

Новая версия создаётся черновиком в `/admin/pricing`, проверяется через pricing
API и публикуется отдельно. Публичная серверная главная и прямой checkout
переключаются только после своих контрольных оплат; включённый каталог сам по
себе не переносит старую Tilda-страницу. Старые платежи не мигрируются и не
пересчитываются.
