...
После трёх последовательных ошибок Cloudflare открывает один incident и отправляет
уведомление владельцу через `sendMessage` отдельного `@TetrisgfgfgfBot`. Cloudflare
...
- недоступна общая `/ready` — один раз запрашивается reboot российской VM через Timeweb API;
...
- через две минуты от первой ошибки останавливаются только перечисленные в runtime
  ID кампаний методом `Campaigns.suspend`;
- после восстановления приходит сообщение, но реклама автоматически не возобновляется
  и остаётся остановленной до ручной проверки владельцем.
...
двух минут Worker может остановить только эти две кампании; автоматически
возобновлять рекламу он не умеет.
...
допускает 100 000 запросов в сутки; одна ежеминутная проверка использует около 1 440.
...
отсутствие дубля, recover закрывает её. Drill использует отдельное Durable Object
