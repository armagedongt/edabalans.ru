...
## Страница индивидуальной оплаты

`https://edabalans.ru/pay` — отдельная серверная страница для заранее
согласованной покупки или услуги. Посетитель сначала указывает сумму, затем в
следующем полноэкранном шаге той же страницы вводит имя, email и комментарий с
назначением платежа; это не всплывающее окно, а кнопка «Назад» возвращает к сумме.
Сервер создаёт
счёт Robokassa через `POST /api/payments/robokassa/manual-checkout`; это не
тариф Мастер-класса и не выдаёт права автоматически. После подтверждения оплаты
показывается сценарий `manual_service`: чек отправляется на email, а Сергей
получает уведомление и вручную согласует консультацию или доступ.
...
В production используется `PRICING_CATALOG_ENABLED=true`: публичный API цен и
обычный серверный checkout читают активную опубликованную версию. Действующая
старая схема продаж Tilda продолжает работать параллельно до отдельного
переключения новой публичной главной.

Для явно согласованного `noindex`-предпросмотра главной также доступен
...
двухчасовую команду корзины. Маршрут не публикуется в OpenAPI; просроченные
неоплаченные checkout удаляются при следующем создании. `noindex` не является
авторизацией, поэтому endpoint остаётся временной границей preview. Новая
публичная главная при отдельном переключении использует обычный
`/api/pricing/site/checkout` либо прямой checkout Robokassa.
...
4. ставит в PostgreSQL-очередь письмо с одноразовыми ссылками Telegram и MAX сроком на 10 дней;
