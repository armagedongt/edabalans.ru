...
| Q-018 | products.dqs | `planned` | Добавить к текущим кнопкам `−/+` дискретную шкалу размера порции: нажатие на значение открывает шкалу с шагом `0,5`, нажатие на деление сразу выбирает и визуально закрашивает соответствующее количество; диапазон и ограничения остаются теми же, что у действующего поля | 2026-09-09 | owner-explicit | Решение владельца отложить доработку интерфейса DQS |
