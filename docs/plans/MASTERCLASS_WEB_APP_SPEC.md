...
Статус: `draft`
Состояние внедрения: требования утверждены; техническая часть уточняется.
...
## 8. Данные редактора и системные поля

Универсальный набор полей курса, дня и материала, а также правила автоматического
формирования оглавления, времени и блока следующего дня принадлежат
`../knowledge-base/modules/masterclass/COURSE_STRUCTURE_CONTRACT.md`. Этот документ
не вводит их вторую копию.

В частности, владелец не заполняет вручную общее время дня и отдельный рекламный
анонс следующего дня: время вычисляется из медиа и материалов, а краткий состав
следующего дня — из его материалов. Технические типы, правила доступа, состояние
публикации и события остаются системными данными.

Специфичный для Мастер-класса признак рецептурного дня может назначать отдельную
визуальную роль в оглавлении, но не доказывает покупку или доступ.

## 9. Специфичное поведение Мастер-класса

Общая композиция дня и место каждого смыслового блока принадлежат
`../knowledge-base/modules/masterclass/COURSE_STRUCTURE_CONTRACT.md`, а оформление —
`../knowledge-base/modules/masterclass/COURSE_VISUAL_SYSTEM.md`.

Точка продаж в текущем Мастер-классе является обычным номерным материалом нужного
дня. Она может визуально выделяться, но не создаёт отдельную секцию
«Дополнительный материал».

Большое задание при необходимости открывается отдельным экраном по
`/day/{n}/task`. Условия завершения могут быть:
...
1. наступило 06:00 после местной календарной даты завершения предыдущего дня;
...
Если участник закончил день до 00:00 по зафиксированному часовому поясу,
продолжение открывается ближайшим утром в 06:00. После полуночи открытие
переносится ещё на одно утро. Повторное открытие не перезапускает таймер. Время и выполнение проверяет сервер,
...
## 18. Граница правил интерфейса

С 24.08.2026 этот технический документ больше не является владельцем общих правил
представления курсов. Канонический формат хранится в двух независимых документах:

- `../knowledge-base/modules/masterclass/COURSE_STRUCTURE_CONTRACT.md` — поля, источники текстов, производные
  представления, порядок блоков, состояния и расчёт времени;
- `../knowledge-base/modules/masterclass/COURSE_VISUAL_SYSTEM.md` — цвета, типографика, карточки, иконки и
  адаптивное оформление.

Прежние указания этого раздела о бренде в шапке, формуле времени, инструкции про
ручные галочки и вертикальной компоновке следующего дня заменены каноническими
документами формата курса. Их применение к production выполняется отдельной задачей
и не происходит автоматически из-за изменения документации.

Специфичная для текущего Мастер-класса последовательность интеграций сохраняется:
после обычных материалов открывается `onboarding-questionnaire`, затем отдельным
шагом — привязка Telegram или MAX. Импортированные изображения не прерывают текст
статьи: они собираются в отдельную галерею внизу материала.
...
- серверный прогресс и правило открытия в 06:00 по местному времени;
- мобильную вёрстку;
...
2. Повторное открытие дня не меняет часовой пояс и рассчитанное время открытия.
