...
Нижний блок следующего дня вертикальный: номер, название, примерное время на
изучение и анонс; затем таймер и кнопка. Незавершённый чек-лист прямо сообщает,
что для завершения дня нужно поставить галочки. После завершения показываются
состояние «День пройден» и точное время открытия следующего дня.
