...
2. Серверная сессия подтверждает личность; на новом устройстве используется
   одноразовый код на email.
3. FastAPI сопоставляет сессию с внутренним `user_id`.
4. PostgreSQL проверяет `user_accesses` и этап открытия ресурса.
5. Прямая ссылка без действующей сессии ведёт на вход, затем возвращает на исходный
   адрес.
6. В URL нельзя помещать email, `user_id`, ответы DQS или права доступа.

В будущем не отправлять пользователю постоянные пароли. Предпочтительная схема:
...
Бренд в шапке приложения — `похудение.рф`, а не «Еда — баланс».
