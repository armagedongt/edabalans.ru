...
→ условие подписки
  ├─ да/заглушка: редактируемый успешный ответ + кнопка на сайт
  └─ нет: редактируемая просьба → 5 минут → повторная проверка
          ├─ да: успешный ответ
          └─ нет: редактируемый fail-open + кнопка на сайт
→ 24 ч → полезный пост дня 2
→ 24 ч → полезный пост дня 3
→ 24 ч → полезный пост дня 4
→ GOTO prepurchase_nurture
```

Текущая редакция использует один общий интенсив на сайте. Будущая редакция заменит
эту часть на четыре поста интенсива и три промежуточных сообщения.

### 6.3. Основная рассылка

Sequence `prepurchase_nurture` начинается с 24-часовой задержки и продающего поста
на пятый день, затем содержит отдельный каркас полезных и продающих сообщений.
