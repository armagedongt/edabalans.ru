...
Проверенная версия кода: commit `04bf9ca`, миграция `20260822_0012`.
Статус выпуска: код и автоматические тесты готовы; production deploy миграции
`0012` ещё не выполнен. До этого сервер продолжает исполнять предыдущую версию.
