...
Одна запись будущего модуля должна выдавать
прямую `t.me` и короткую `edabalans.ru/t/` формы одного кода. Учёт web-click до
нажатия Start владельцу сейчас не требуется; свободные UTM необязательны.
Отдельно согласована, но не реализована атрибуция пути «уникальная invite-ссылка
канала → вступление/заявка → поздний старт бота»; полное ТЗ находится в
`plans/TELEGRAM_CHANNEL_ATTRIBUTION_SPEC.md`.
...
- события `chat_member`/`chat_join_request` и связь источника канала с последующим
  запуском бота.
