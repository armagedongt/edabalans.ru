...
4. существует ли active/waiting run стартовой sequence или `welcome_intensive`;
...
- первый вход или новая рассылка ещё не запускалась → запустить стартовую
  последовательность (навигация, кружок, приветствие, кнопка);
...
Техническая sequence `start_attribution_entry` исполняет только новый вход после
проверок `start_router.py`:

```text
навигационный пост + закреп
→ видеокружок
→ «Сделайте похудение проще» + кнопка
→ WAIT_BUTTON
→ заглушка проверки подписки
→ при отказе напоминание + 5 минут + fail-open
→ GOTO welcome_intensive
```
...
### 6.2. Welcome — ровно 8 сообщений

Sequence `welcome_intensive` начинается с первого дня:

| № | Сообщение | Задержка |
|---:|---|---:|
| 1 | День 1 | сразу |
| 2 | промежуточный пост 1 | 12 часов |
| 3 | День 2 | 11 часов |
| 4 | промежуточный пост 2 | 12 часов |
| 5 | День 3 | 12 часов |
| 6 | промежуточный пост 3 | 12 часов |
| 7 | День 4 | 12 часов |
| 8 | промежуточный пост 4 | 12 часов |

После восьмого сообщения переход `GOTO` запускает `prepurchase_nurture`.

### 6.3. Основная рассылка

Sequence `prepurchase_nurture` содержит отдельный каркас продающих и полезных
постов после интенсива. Она больше не смешана со Start и восьмью сообщениями
Welcome. Порядок и тексты владелец редактирует независимо.
