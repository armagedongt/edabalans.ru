...
на 24.08.2026; основной бот и канал проверены через Telegram Bot API.
...
До разбора payload и запуска Start-router действует временный production-
предохранитель. Контакт сохраняется всегда, но при
`TELEGRAM_MAINTENANCE_MODE=true` только ID из
`TELEGRAM_MAINTENANCE_ALLOWED_USER_IDS` проходят дальше. Остальные получают
`tpl_maintenance_notice`, статус `maintenance_waitlist` и событие
`maintenance_contact`; M-link, атрибуция и sequence для них не исполняются.
...
```text
навигация + закреп → кружок → большой пост + кнопка → WAIT_BUTTON
→ условие подписки
  ├─ да: сразу День 1
