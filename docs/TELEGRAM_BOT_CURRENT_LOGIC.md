...
### 5.1. Нативные приложения внутри Telegram

Кнопка с типом `web_app` может открыть тот же публичный адрес приложения, который
работает в обычном браузере: `/dqs`, `/training`, `/metabolism` или `/recipes`.
Клиент передаёт Telegram `initData`, backend проверяет его подпись и свежесть,
находит уже подключённый `MessengerAccount`, затем отдельно проверяет право на
запрошенный ресурс. Только после этого создаётся обычная серверная сессия ЛК.

Команда или видимость кнопки не является авторизацией: пересланная ссылка без
действующего доступа либо без подписанного `initData` приложение не открывает.
В обычном браузере тот же адрес использует сохранённую cookie-сессию, а при её
отсутствии отправляет пользователя на `/lk`.

Служебный payload `training_admin` открывает мобильный вход в админку тренировок
по адресу `/admin/strength?mobile=1`. Сама ссылка не авторизует администратора:
данные клиентов доступны только после обычного серверного входа в админку и
проверки её cookie-сессии. В мобильном выборе профиля показываются только клиенты,
у которых уже есть записи тренировок; поиск работает по имени и email, а работа
с выбранным профилем идёт через штатный managed-режим с аудитом изменений.
...
- после отправки опроса дня 2 по продуктовым категориям помещает в тот же outbox
  ...
  если Telegram ещё не подключён, задача ждёт связи, а повторная отправка формы не
  создаёт дубль;
...
Бот резервирует пять коротких общих payload, которые не являются tracking-ссылками
и не запускают welcome:

- `apps` — все доступные пользователю приложения;
- `dqs` — система оценки качества питания;
- `training` — силовые тренировки;
- `metabolism` — калькулятор метаболизма;
- `recipes` — калькулятор рецептов.

Отдельный служебный payload `training_admin` не входит в пользовательское меню
приложений: он отдаёт администратору кнопку мобильной админки тренировок. Знание
payload или пересылка кнопки не дают доступа к данным без серверной админ-сессии.

После `/start` бот связывает контакт с существующим `messenger_accounts` по точному
Telegram ID и читает действующие права в PostgreSQL. Кнопка приложения появляется
только при наличии соответствующего права. Сам Mini App при открытии ещё раз
проверяет подписанный Telegram `initData` и то же серверное право. Поэтому скрытая
кнопка не является границей безопасности.

Кнопка «Обновить» повторно читает права и редактирует то же сообщение через
`editMessageText`. Эти служебные входы разрешены при общем режиме ремонта бота, но
не запускают цепочки и не обходят проверку доступа.
