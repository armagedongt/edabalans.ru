...
В локальной версии подключён отдельный вход покупателя по одноразовой M-ссылке.
До deploy production продолжает работать по состоянию, указанному в шапке этого
документа.

## 6. Три отдельные цепочки до покупки

Локально подготовлена структура из трёх модулей. До deploy production продолжает
работать на предыдущем коммите; этот раздел описывает новую проверенную структуру.
