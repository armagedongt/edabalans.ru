...
кода в исходном браузере; пять ошибок блокируют попытку.
Пока бот находится в maintenance-режиме, путь работает только для серверного
allowlist владельца и не запускает массовые цепочки.
...
- после привязки Telegram по M-ссылке помещает в outbox идентификационное сообщение
  и анкету;
- после отправки опроса дня 2 по продуктовым категориям помещает в тот же outbox
  одно сообщение `tpl_postpurchase_current_diet` с заполненным нумерованным списком;
  если Telegram ещё не привязан, задача ждёт связи, а повторная отправка формы не
  создаёт дубль;
