...
Проверенная версия: commit `fcda599`, миграция `20260823_0020`.
Статус выпуска: развёрнуто на тестовом боте `@TetrisgfgfgfBot`; post-purchase
ограничен выбранными тестовыми профилями.
...
Отдельный вход покупателя по одноразовой M-ссылке развёрнут на тестовом боте.
Он связывает Telegram-аккаунт с уже существующим CRM user и запускает центральную
обработку покупки.

## 6. Три отдельные цепочки до покупки

Структура из трёх модулей развёрнута на тестовом боте и является текущей
...
Полный граф условий первого/повторного Start показан в админке отдельной карточкой
и описан в `knowledge-base/modules/telegram/START_WELCOME_ROUTING.md`.
...
Отключённая проверка пропускает пользователя дальше и записывает
`subscribed=null`, не выдавая заглушку за реальную подписку. Тайм-аут WAIT_BUTTON
хранится в `tg_sequence_runs.next_action_at`; scheduler продолжает ветку через
явную edge `timeout`.

До подключения `getChatMember` бот:

- не вызывает Telegram `getChatMember`;
- не знает ID проверяемого канала;
- не получает фактический статус из Telegram;
- показывает ветвление в графе, но в живом тесте проходит по ветке заглушки.

Каждый результат хранится в `tg_tracking_events`; известный реальный статус также
обновляет `messenger_accounts.subscription_status` и `subscription_checked_at`.
...
последовательным run. На production тестового бота этот dispatch пока разрешён только
