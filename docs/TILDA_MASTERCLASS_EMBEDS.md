...
На публичных Tilda-страницах приложений, включая `/dqs`, загрузчик после готовности
документа сам подключает официальный `tilda-members-init.min.js` только как reader
уже сохранённого профиля. Его собственная маршрутизация страницы при этом не
запускается. Это позволяет прямой ссылке определить действующий вход, а при его
отсутствии сохранить внутренний адрес, пройти штатный `/members/login` и один раз
вернуться в приложение без цикла через кабинет.
...
20-дневного курса их не вставлять: анкета, DQS, рецепты, саморевью и персональные
