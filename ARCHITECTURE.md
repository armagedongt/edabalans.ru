...
Для текущего личного кабинета это жёсткое правило: закрытая Tilda Members Area —
единственный пользовательский вход. Frontend берёт email из
`tma__getProfileObjFromLS().login` и не показывает повторную форму email, пароль,
код из письма или подтверждение отдельной пользовательской сессии. Такие механизмы
можно вводить только отдельной явно согласованной задачей владельца. Backend при
этом по-прежнему сопоставляет email с внутренним `user_id` и проверяет продуктовые
права в PostgreSQL.
