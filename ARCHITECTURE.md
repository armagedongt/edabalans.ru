...
## Реализованный фундамент на 06.09.2026
...
- NocoDB 2026.08.0 только на loopback/через SSH, без публичного адреса;
...
уже работают через единый origin `https://edabalans.ru`. Tilda пока остаётся
публичным сайтом и корзиной на `похудение-это-есть.рф`.
...
Сейчас основной технический домен — `edabalans.ru`. Функции разделяются путями:

```text
edabalans.ru/api/*       → backend / интеграции
edabalans.ru/apps/*      → frontend-приложения
edabalans.ru/lk          → личный кабинет
edabalans.ru/intensive   → бесплатный интенсив
edabalans.ru/admin/*     → специализированные админки
```

Текущий публичный сайт продолжает работать на `похудение-это-есть.рф` и Tilda.
Новые рекламные и бот-ссылки интенсива используют единый origin, но несколько
legacy CTA и ресурсов всё ещё обращаются к `app.`/`go`; они перечислены в
`docs/DOMAIN_CUTOVER_RUNBOOK.md`. Следующий перенос должен удалить эти последние
runtime-зависимости до отключения старых host.

После полного ухода с Tilda публичный сайт, личный кабинет и пользовательские
интерфейсы переезжают на `похудение.рф/<путь>`. Административный кабинет владельца
не обязан переезжать вместе с ними: его согласованный постоянный host —
`admin.edabalans.ru`, а текущие `/admin` и `/crm` на `edabalans.ru` сохраняются до
отдельного переключения. Целевая роль `api.edabalans.ru` — только машинная
совместимость API и webhook, а не browser-интерфейсы. Сейчас общий Caddy catch-all
технически пропускает через этот host и другие backend-маршруты; это legacy-
совместимость, а не канонический адрес интерфейсов. Новые публичные API после
переезда получают пути `похудение.рф/api/*`; `api.` может остаться временным
совместимым host.
`app.edabalans.ru` не используется для админки; явно оставленные самостоятельные
приложения, включая историческую погоду, сохраняются там до отдельного решения.
...
короткий загрузочный код
    ↓
JS/CSS и API с edabalans.ru
```

Код приложения живёт в GitHub и на сервере, а Tilda только подключает его в страницу.

### Общий контракт загрузки медиа

Все новые и изменяемые публичные страницы, статьи, материалы курсов и приложения
задают стратегию загрузки медиа в источнике разметки или общем renderer. Изображения
и iframe ниже первого экрана по умолчанию получают `loading="lazy"`; изображения —
также `decoding="async"`. Ключевое изображение или видео первого экрана явно получает
`loading="eager"`, а главное LCP-изображение — `fetchpriority="high"`. Поэтому lazy
load не включается без разбора для всего документа: он не должен задерживать первый
экран. Динамически создаваемые галереи и вставляемые серверные страницы следуют тому
же правилу до добавления элементов в видимый DOM.

Клиентские приложения определяют участника только по собственной непрозрачной
серверной сессии на `https://edabalans.ru/lk`. Email из страницы, query-параметра
или localStorage не является удостоверением личности. У каждого человека один
внутренний `user_id`, а покупки, прогресс и продуктовые права читаются только из
PostgreSQL.

После подтверждённой оплаты backend независимо от источника платежа ставит в
очередь письмо с одноразовыми 24-часовыми ссылками в Telegram и MAX. Первый
выбранный мессенджер связывается с `user_id` и выдаёт логин и пароль. Страница
успешной оплаты только сообщает проверить почту и не является доказательством
оплаты или источником доступа.
...
Tilda отправляет подтверждённые заказы на backend через webhook:

```text
Tilda
    ↓
https://edabalans.ru/api/integrations/tilda/payments
    ↓
PostgreSQL
```
...
Удалённый `main` содержит только готовый проверенный результат. Режим общей
локальной папки, очередь записи и исключения для изолированной реализации
определены в `docs/CHAT_WORKSTREAMS.md`. Отдельный worktree на каждый разговор
не обязателен. Смешанный старый checkout не выпускается целиком: принятый набор
переносится поверх свежего `origin/main` в безопасном изолированном окружении.
...
## 37. Исходный порядок реализации (выполнен)
...
6. Привязка единого публичного origin edabalans.ru.
7. Docker.
8. PostgreSQL.
9. FastAPI skeleton.
10. /health.
