...
Основной технический домен: `edabalans.ru`. Функции разделяются путями, а не
доменами третьего уровня:
...
становится `похудение.рф/<путь>`. Тогда `edabalans.ru` и любые поддомены не
используются для сайтов, приложений, frontend или API. Это не меняет существующие
адреса само по себе: `app.edabalans.ru/weather` сохраняется до отдельного решения
о его переносе.
...
Рабочая папка `main` не служит черновиком. Каждый незавершённый поток ведётся в
отдельной ветке и worktree от актуального `origin/main`; в `main` объединяется
только готовый проверенный результат. Если в основной папке уже накопились
незавершённые файлы, они сначала сохраняются отдельно, а выпуск собирается из
нового чистого worktree. Это исключает случайный перенос чужих правок в deploy.
