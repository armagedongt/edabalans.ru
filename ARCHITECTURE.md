...
В переходном личном кабинете работают два адаптера входа в одну серверную учётную
запись. Старый путь получает email из авторизованного профиля Tilda. Новый путь
принимает email и пароль на `https://edabalans.ru/lk`, создаёт собственную
непрозрачную серверную сессию и после входа открывает тот же кабинет. Оба пути
сопоставляются с одним внутренним `user_id` и читают одни продуктовые права из
PostgreSQL; отдельного «нового кабинета» и копии материалов нет.
