...
function renderTelegramEditor(host, content, onSave) {
  ...
  host.innerHTML = `<div class="tg-top"><div class="tg-avatar">е</div><div><b>Еда — это баланс</b><small>бот</small></div></div><div class="editor-label">ТЕКСТ СООБЩЕНИЯ — НАЖМИТЕ В ПОЛЕ НИЖЕ</div><div class="tg-chat"><div class="tg-bubble">${media}<div id="tg-editor" class="tg-text" contenteditable="true">${telegram(content.body_source || '')}</div><time>сейчас ✓✓</time></div></div><div class="composer-tools" id="format-tools">${editorToolbar()}</div><div class="composer-actions"><label class="file-action">＋ Загрузить медиа<input id="media-file" type="file" accept="image/*,video/mp4,audio/*"></label><select id="media-kind" title="Как отправить прикреплённый файл"><option value="" ${!content.media_kind?'selected':''}>Без медиа</option><option value="photo" ${content.media_kind==='photo'?'selected':''}>Фото</option><option value="video" ${content.media_kind==='video'?'selected':''}>Обычное видео</option><option value="video_note" ${content.media_kind==='video_note'?'selected':''}>Видеокружок</option><option value="voice" ${content.media_kind==='voice'?'selected':''}>Голосовое</option></select><button class="action" id="save-message">Сохранить сообщение</button></div><div class="meta composer-status" id="composer-status">Текст и тип медиа сохраняются только после кнопки «Сохранить сообщение»</div>`;
  ...
  host.querySelector('#save-message').onclick = async event => { content.media_kind=host.querySelector('#media-kind').value||null; await onSave({body_source:editor.innerHTML, media_kind:content.media_kind, media_path:content.media_path || null}); toast(event.currentTarget); };
}

async function sequences() {
  busy(); const list = await api('/admin/sequences');
  root.innerHTML = `<div class="help-card"><b>Три последовательных модуля</b><span>Start содержит источники и развилки. Welcome содержит навигацию, кружок, кнопку, подписку и первые четыре дня. Основная рассылка начинается с продажи на пятый день.</span></div><div class="grid sequence-cards">${list.map(item => `<article class="card"><span class="eyebrow">${item.item_type==='module'?'ГЛОБАЛЬНЫЙ МОДУЛЬ':item.status === 'published' ? 'АКТИВНАЯ ЦЕПОЧКА' : 'ЗАГОТОВКА'}</span><h3>${esc(item.name)}</h3><p class="meta">${esc(item.description || 'Описание будет добавлено')}</p><div class="card-footer"><span>${item.steps} блоков · версия ${item.version}</span><button class="action" data-code="${esc(item.code)}" data-kind="${esc(item.item_type||'sequence')}">Открыть</button></div></article>`).join('')}</div>`;
  root.querySelectorAll('[data-code]').forEach(button => button.onclick = () => button.dataset.kind==='module' ? mapView(null,button.dataset.code) : sequence(button.dataset.code));
}
async function sequence(code) {
  busy(); const data = await api(`/admin/sequences/${code}`); title.textContent = data.name;
  root.innerHTML = `<section class="rule-card"><div class="sequence-actions"><button class="back-link" id="back">← Цепочки</button><button class="action" id="show-map">Открыть схему</button></div><div><span>ЗАПУСК</span><p>${esc(data.rule.start)}</p></div><div><span>ОСТАНОВКА</span><p>${esc(data.rule.stop)}</p></div><div><span>ДАЛЬШЕ</span><p>${esc(data.rule.next)}</p></div></section><div class="sequence-workspace"><section class="flow-list" id="flow-list">${data.steps.map(step => { const human = humanStep(step), delay = delayDisplay(step.delay_seconds); return `<article class="flow-node ${step.kind.toLowerCase()} ${step.enabled ? '' : 'disabled'}" data-step="${step.id}"><div class="node-head"><span class="node-number">${step.position}</span><span class="node-kind">${esc(step.kind)}</span>${delay ? `<b>${delay.value} ${delay.unit}</b>` : ''}</div><h3>${esc(human.title)}</h3><p>${esc(human.note)}</p>${conditionBranches(step)}</article>`; }).join('')}</section><aside class="telegram-pane" id="step-pane"><div class="empty">Нажмите на текстовый блок — справа откроется редактор. Логика и стрелки меняются в коде и тестах.</div></aside></div>`;
