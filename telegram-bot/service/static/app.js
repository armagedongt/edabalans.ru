...
function renderTelegramEditor(host, content, onSave, presentation=null) {
  ...
  const textEditor = `<textarea id="tg-editor" class="tg-text html-editor" spellcheck="true">${esc(content.body_source || '')}</textarea>`;
  const usageText=(content.usages||[]).map(usage=>`${usage.module||'модуль'}: ${usage.previous||'вход'} → ${usage.label||usage.step||'сообщение'} → ${usage.next||'выход'}`).join('\n')||'Место использования пока не найдено.';
  const issueText=(content.issues||[]).join(', ')||'ошибок готовности нет';
  const authoring = `<section class="trigger-help authoring-brief"><b>Редакционное задание</b><small>Версия ${esc(content.content_version || '—')} · runtime: ${esc(content.runtime_status || '—')} · готовность: ${esc(content.editorial_status || '—')} · проверка: ${esc(content.runtime_readiness || '—')} (${esc(issueText)})</small><label>Цель сообщения<input id="content-purpose" maxlength="2000" value="${esc(content.purpose || '')}"></label><label>ТЗ писателю<textarea id="content-brief" maxlength="10000">${esc(content.writer_brief || '')}</textarea></label><label>Место в логике<textarea readonly>${esc(usageText)}</textarea></label><label>Готовность<select id="editorial-status"><option value="placeholder" ${content.editorial_status==='placeholder'?'selected':''}>Заглушка</option><option value="needs_writing" ${content.editorial_status==='needs_writing'?'selected':''}>Нужно написать</option><option value="draft" ${content.editorial_status==='draft'?'selected':''}>Черновик</option><option value="approved" ${content.editorial_status==='approved'?'selected':''}>Подтверждено — писателю не трогать</option></select></label><small>Подтверждённые сообщения массовый модуль-писатель пропускает.</small></section>`;
  host.innerHTML = `<div class="tg-top"><div class="tg-avatar">е</div><div><b>Еда — это баланс</b><small>бот</small></div></div>${triggerHelp}${authoring}<div class="editor-label">ТЕКСТ СООБЩЕНИЯ · TELEGRAM HTML</div><div class="tg-chat"><div class="tg-bubble">${media}${textEditor}<time>сейчас ✓✓</time></div></div>${buttonEditor}<div class="composer-actions"><label class="file-action">＋ Загрузить медиа<input id="media-file" type="file" accept="image/*,video/mp4,audio/*"></label><select id="media-kind" title="Как отправить прикреплённый файл"><option value="" ${!content.media_kind?'selected':''}>Без медиа</option><option value="photo" ${content.media_kind==='photo'?'selected':''}>Фото</option><option value="video" ${content.media_kind==='video'?'selected':''}>Обычное видео</option><option value="video_note" ${content.media_kind==='video_note'?'selected':''}>Видеокружок</option><option value="voice" ${content.media_kind==='voice'?'selected':''}>Голосовое</option></select><button class="action" id="save-message">Сохранить сообщение</button></div><div class="meta composer-status" id="composer-status">Текст, ТЗ, готовность, медиа и подпись кнопки сохраняются только после кнопки «Сохранить сообщение»</div>`;
  const editor = host.querySelector('#tg-editor');
  const markChanged=()=>{const readiness=host.querySelector('#editorial-status');if(readiness.value==='approved'){readiness.value='draft';host.querySelector('#composer-status').textContent='Текст изменён. Чтобы сохранить его в живой слот, подтвердите новую редакцию в поле «Готовность».';}};
  [editor,host.querySelector('#content-purpose'),host.querySelector('#content-brief')].forEach(field=>field.addEventListener('input',markChanged));
  ...
  host.querySelector('#save-message').onclick = async event => { const status=host.querySelector('#composer-status'); content.media_kind=host.querySelector('#media-kind').value||null; const changes={body_source:editor.value,source_format:'telegram_html',purpose:host.querySelector('#content-purpose').value.trim(),writer_brief:host.querySelector('#content-brief').value.trim(),editorial_status:host.querySelector('#editorial-status').value,media_kind:content.media_kind,media_path:content.media_path || null,...(Number.isInteger(content.content_version)?{expected_version:content.content_version}:{})}; try{await onSave(changes, host.querySelector('#button-text')?.value.trim() || null); Object.assign(content,changes); if(Number.isInteger(content.content_version))content.content_version+=1; toast(event.currentTarget); status.textContent='Сохранена подтверждённая версия сообщения.';}catch(error){status.textContent=error.message;} };
}
...
async function contacts() {
  ...
  const draw = () => {
    ...
    document.querySelectorAll('.message').forEach(button => button.onclick = () => openConversation(button.dataset.id));
  };
...
async function openConversation(contactId) {
  const timeline=await api(`/admin/contacts/${contactId}/timeline`), modal=document.createElement('div');modal.className='modal-backdrop';
  const messages=timeline.map(item=>`<article class="dialog-message ${item.direction}"><small>${esc(item.kind)} · ${dateText(item.occurred_at)} · ${esc(item.status)}</small><div>${esc(item.body||'')}</div></article>`).join('')||'<div class="empty">Сообщений пока нет</div>';
  modal.innerHTML=`<section class="modal"><button type="button" class="modal-close">×</button><span class="eyebrow">TELEGRAM TIMELINE</span><h2>Диалог и автоматические отправки</h2><div class="dialog-timeline">${messages}</div><form id="dialog-reply"><div class="simple-editor" contenteditable="true" id="personal-text"></div><button class="action">Отправить в Telegram</button></form></section>`;document.body.append(modal);modal.querySelector('.modal-close').onclick=()=>modal.remove();modal.onclick=event=>{if(event.target===modal)modal.remove();};
  modal.querySelector('#dialog-reply').onsubmit=async event=>{event.preventDefault();const text=modal.querySelector('#personal-text').innerHTML.trim();if(!text)return;await api(`/admin/contacts/${contactId}/messages`,{method:'POST',body:JSON.stringify({text})});modal.remove();openConversation(contactId);};
}

async function broadcasts() {
  busy(); const [rows,contacts,tags]=await Promise.all([api('/admin/broadcasts'),api('/admin/contacts'),api('/admin/tags')]);
  root.innerHTML=`<div class="help-card"><b>Безопасный порядок</b><span>Черновик → показать точное число людей → тест owner-аккаунту → подтвердить число → отправить сейчас или запланировать. Во время ремонта аудитория всегда ограничена двумя вашими аккаунтами.</span></div><div class="broadcast-workspace"><section><div class="section-heading"><div><span class="eyebrow">РАЗОВЫЕ РАССЫЛКИ</span><h2>Сохранённые рассылки</h2></div><button class="action" id="new-broadcast">＋ Новая</button></div><div class="broadcast-list">${rows.map(row=>`<article class="card broadcast-card" data-broadcast="${row.id}"><div><h3>${esc(row.title)}</h3><span class="meta">${esc(row.status)} · отправлено ${row.sent} · ошибок ${row.failed}</span></div><div class="contact-actions">${row.status==='draft'?`<button class="action preview-broadcast" data-id="${row.id}">Проверить аудиторию</button>`:''}${row.failed?`<button class="action alt retry-broadcast" data-id="${row.id}">Повторить ошибки</button>`:''}</div></article>`).join('')||'<div class="empty">Рассылок пока нет</div>'}</div></section><aside class="telegram-pane" id="broadcast-editor"></aside></div>`;
  const editor=document.querySelector('#broadcast-editor');
  const newBroadcast=()=>{const content={body_source:'',media_kind:null,media_path:null};editor.innerHTML=`<div class="broadcast-fields"><input id="bc-title" placeholder="Название рассылки"><input id="bc-ids" placeholder="Telegram ID через запятую — пусто значит все active"><select id="bc-tags" multiple title="Все выбранные теги обязательны">${tags.map(tag=>`<option value="${esc(tag.id)}">${esc(tag.name)}</option>`).join('')}</select><input id="bc-button-text" placeholder="Текст кнопки — необязательно"><input id="bc-button-url" placeholder="https:// ссылка кнопки"></div><div id="bc-telegram"></div>`;renderTelegramEditor(editor.querySelector('#bc-telegram'),content,async changes=>{Object.assign(content,changes);const titleValue=editor.querySelector('#bc-title').value.trim();if(!titleValue)throw new Error('Укажите название рассылки');const telegram_user_ids=editor.querySelector('#bc-ids').value.split(',').map(value=>value.trim()).filter(Boolean),tag_ids=[...editor.querySelector('#bc-tags').selectedOptions].map(option=>option.value),buttonText=editor.querySelector('#bc-button-text').value.trim(),buttonUrl=editor.querySelector('#bc-button-url').value.trim(),buttons=buttonText||buttonUrl?[{text:buttonText,url:buttonUrl}]:[];await api('/admin/broadcasts',{method:'POST',body:JSON.stringify({title:titleValue,text:content.body_source,media_kind:content.media_kind,media_path:content.media_path,segment:{status:'active',telegram_user_ids,tag_ids},buttons})});broadcasts();});};
  document.querySelector('#new-broadcast').onclick=newBroadcast;
  document.querySelectorAll('.broadcast-card').forEach(card=>card.onclick=event=>{if(event.target.closest('button'))return;const row=rows.find(value=>value.id===card.dataset.broadcast);renderTelegramEditor(editor,row,async()=>{throw new Error('После сохранения создавайте новую рассылку, чтобы история не менялась');});});
  document.querySelectorAll('.preview-broadcast').forEach(button=>button.onclick=async()=>{const preview=await api(`/admin/broadcasts/${button.dataset.id}/preview`),options=contacts.map(contact=>`<option value="${esc(contact.id)}">${esc(contact.name||contact.username||contact.telegram_user_id)}</option>`).join(''),modal=document.createElement('div');modal.className='modal-backdrop';modal.innerHTML=`<section class="modal"><button class="modal-close">×</button><span class="eyebrow">PREVIEW АУДИТОРИИ</span><h2>${preview.recipient_count} получателей</h2><p class="meta">${preview.maintenance_limited?'Сейчас действует режим ремонта: войдут только owner-аккаунты.':'Массовый режим: внимательно проверьте число.'}</p><select id="bc-test-contact">${options}</select><div class="contact-actions"><button class="action" id="bc-test">Отправить тест</button><button class="action" id="bc-send">Отправить ${preview.recipient_count}</button><button class="action alt" id="bc-schedule">Запланировать</button></div></section>`;document.body.append(modal);modal.querySelector('.modal-close').onclick=()=>modal.remove();modal.querySelector('#bc-test').onclick=async()=>{await api(`/admin/broadcasts/${button.dataset.id}/test`,{method:'POST',body:JSON.stringify({contact_id:modal.querySelector('#bc-test-contact').value})});toast(modal.querySelector('#bc-test'));};modal.querySelector('#bc-send').onclick=async()=>{if(!confirm(`Точно отправить ${preview.recipient_count} людям?`))return;await api(`/admin/broadcasts/${button.dataset.id}/launch`,{method:'POST',body:JSON.stringify({confirmed_recipient_count:preview.recipient_count})});modal.remove();broadcasts();};modal.querySelector('#bc-schedule').onclick=async()=>{const when=prompt('Дата и время, например 2026-08-25T12:00:00+03:00');if(!when)return;await api(`/admin/broadcasts/${button.dataset.id}/schedule`,{method:'POST',body:JSON.stringify({confirmed_recipient_count:preview.recipient_count,scheduled_at:when})});modal.remove();broadcasts();};});
  document.querySelectorAll('.retry-broadcast').forEach(button=>button.onclick=async()=>{if(!confirm('Повторить только неудачные доставки?'))return;await api(`/admin/broadcasts/${button.dataset.id}/retry`,{method:'POST'});broadcasts();});newBroadcast();
}
