...
async function sequences() {
  ...
  root.innerHTML = `<div class="help-card"><b>Три последовательных модуля</b><span>Start содержит входы, атрибуцию и развилки. Welcome содержит 8 сообщений интенсива. Основная рассылка начинается только после Welcome.</span></div><div class="grid sequence-cards">${list.map(item => `<article class="card"><span class="eyebrow">${item.item_type==='module'?'ГЛОБАЛЬНЫЙ МОДУЛЬ':item.status === 'published' ? 'АКТИВНАЯ ЦЕПОЧКА' : 'ЗАГОТОВКА'}</span><h3>${esc(item.name)}</h3><p class="meta">${esc(item.description || 'Описание будет добавлено')}</p><div class="card-footer"><span>${item.steps} блоков · версия ${item.version}</span><button class="action" data-code="${esc(item.code)}" data-kind="${esc(item.item_type||'sequence')}">Открыть</button></div></article>`).join('')}</div>`;
