...
def test_account_claim_creates_password_once_and_queues_questionnaire(tmp_path):
    ...
    with Session(engine) as session:
        ...
        session.add(repeat_onboarding)
        session.flush()
        session.add(
            MessengerLinkToken(
                user_id=target.id,
                account_onboarding_id=repeat_onboarding.id,
                platform="telegram",
                purpose="account_credentials",
                token_hash=hashlib.sha256(repeat_payload.encode("ascii")).hexdigest(),
                expires_at=datetime.now(UTC) + timedelta(minutes=5),
            )
        )
        session.commit()

        handled, repeat_reply = consume_masterclass_link(
            session,
            contact,
            {"id": 42, "username": "owner", "first_name": "Сергей"},
            repeat_payload,
        ...
        assert handled is True
        assert "https://go.example.test/lk" in repeat_reply
        assert "Пароль:" not in repeat_reply
        assert "Пароль уже приходил" in repeat_reply
        assert "06.09.2026" in repeat_reply


def test_free_account_claim_does_not_queue_paid_questionnaire(tmp_path):
    engine = make_engine(f"sqlite:///{tmp_path / 'free-account-claim.sqlite'}")
    Base.metadata.create_all(engine)
    with Session(engine) as session:
        for ddl in (
            "CREATE TABLE user_emails (id TEXT, user_id TEXT, email_original TEXT, email_normalized TEXT, is_primary BOOLEAN, created_at DATETIME)",
            ...
            session.execute(text(ddl))
        target = CrmUser(display_name="Новый пользователь", status="active", data_origin="native")
        placeholder = CrmUser(display_name="Telegram", status="active", data_origin="native")
        bot = BotInstance(code="test", username="test", display_name="test", token_env_name="TOKEN", is_active=True)
        session.add_all([target, placeholder, bot])
        session.flush()
        session.execute(
            text(
                ...
                "(id,user_id,email_original,email_normalized,is_primary,created_at) "
                "VALUES ('e1',:user_id,'member@example.test','member@example.test',1,CURRENT_TIMESTAMP)"
            ),
            {"user_id": target.id},
        )
        contact = Contact(
        ...
        assert session.scalar(
            select(MasterclassNotification.id).where(
                MasterclassNotification.user_id == target.id
            )
        ) is None


def test_existing_password_hint_points_to_the_original_issue_date():
