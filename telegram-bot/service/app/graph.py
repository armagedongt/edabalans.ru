...
def start_attribution_graph(session: Session) -> dict[str, Any]:
    ...
    nodes = [
        ...
        node("attribution", "technical", "Распознать источник и first-touch", "Сохранить до заглушки; не перезаписывать повторным Start", 6, Хранение="tg_tracking_* / attribution_events / user_tags"),
        node("purchase_fact", "technical", "Тихо определить наличие мастер-класса", "Платёж или активное право ACCESS_MASTERCLASS", 7, Исполнение="app/start_router.py"),
        node("maintenance_gate", "condition", "Разрешён полный доступ во время ремонта?", "Только два Telegram ID владельца", 8, Исполнение="app/maintenance.py"),
    ...
    edges = [
        ...
        {"id": "e05a", "source": "attribution", "target": "purchase_fact", "label": "Источник сохранён", "branch": "default"},
        {"id": "e05d", "source": "purchase_fact", "target": "maintenance_gate", "label": "Факт определён", "branch": "default"},
        {"id": "e05e", "source": "maintenance_gate", "target": "has_masterclass", "label": "Да", "branch": "true"},
...
def apps_menu_graph() -> dict[str, Any]:
    nodes = [
        ...
        {"id": "apps_access", "kind": "condition", "label": "Есть действующий доступ?", "subtitle": "Только active user_accesses", "position": 3, "details": {"Источник": "resources + user_accesses"}},
        {"id": "apps_buttons", "kind": "message", "label": "Показать доступные приложения", "subtitle": "Одна кнопка или меню всех приложений", "position": 4, "details": {"Безопасность": "Mini App повторно проверяет initData и доступ"}},
    ...
    edges = [
        ...
        {"id": "apps:3", "source": "apps_access", "target": "apps_buttons", "label": "Да", "branch": "true"},
        {"id": "apps:4", "source": "apps_access", "target": "apps_denied", "label": "Нет", "branch": "false"},
