...
class MessengerLinkToken(Base):
    ...
    id: Mapped[str] = mapped_column(Uuid(as_uuid=False), primary_key=True, default=uuid_text)
    user_id: Mapped[str] = mapped_column(Uuid(as_uuid=False), nullable=False, index=True)
    account_onboarding_id: Mapped[str | None] = mapped_column(
        Uuid(as_uuid=False), ForeignKey("account_onboardings.id", ondelete="CASCADE"), index=True
    )
    platform: Mapped[str] = mapped_column(String(32), nullable=False)
...
class AccountOnboarding(Base):
    ...
    email_status: Mapped[str] = mapped_column(String(32), default="pending", nullable=False)
