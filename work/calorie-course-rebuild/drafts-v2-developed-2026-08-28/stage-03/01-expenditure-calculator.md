...
Сам смысл понятен: одинаковое количество медленных перемещений по квартире и бодрой ходьбы на улице не обязано давать одинаковый расход. Но бытовой критерий выбора процента в текущем интерфейсе пока не объяснён достаточно ясно.

[НУЖНА СВЕРКА С КАЛЬКУЛЯТОРОМ: дать человеку однозначное бытовое правило выбора 0/25/50/75/100% активных шагов и проверить, что оно совпадает с действующей формулой]

До этой сверки не изображайте точность. Если поле вызывает сомнение, сделайте два варианта с соседними значениями и сохраните получившийся диапазон. Это честнее, чем уверенно выбрать 75%, потому что сегодня вы шли в магазин довольно бодро.
...
Поле требует отдельной методической сверки. До неё используйте соседние сценарии и сохраните диапазон, а не одну цифру.
...
[НУЖЕН АКТУАЛЬНЫЙ СКРИНШОТ: исходные поля текущего калькулятора с нулевым дефицитом]

[НУЖЕН АКТУАЛЬНЫЙ СКРИНШОТ: результат и сравнение двух вариантов текущего калькулятора]
...
- [Diet induced thermogenesis](https://pubmed.ncbi.nlm.nih.gov/15534426/)
