...
## Дополнение 2026-09-10: факты для финальной доводки

Предыдущий раздел отражает раннюю архитектуру и частично устарел. Текущее
состояние после последних выпусков:

- `products.public-site` уже зарегистрирован и владеет
  `backend/app/static/homepage-preview/**`, `homepage.js`, Tilda shell, публичным
  content API и browser-тестами.
- Каноническая новая главная — `homepage-preview/mobile.html`; действующий
  `/preview/homepage-mobile` и Tilda loader используют это же полотно.
- Новая главная уже читает канонический pricing catalog и создаёт прямой
  Robokassa checkout по `price_code`; раннее решение оставить новую главную на
  корзине Tilda отменено в `decisions.md`.
- `site-cookie-notice.js` сейчас хранит только `accepted-v1`. Отказ,
  versioned receipt, отзыв решения и управление загрузкой Метрики отсутствуют.
- Интенсив запускает счётчик 97331502 независимо от cookie-компонента, а VSL
  создаёт viewer UUID/localStorage. Внутренний прогресс, media analytics и
  публичную Метрику нельзя считать одним видом данных: shell должен включать
  только согласованный необязательный слой, не ломая продуктовый прогресс.
- Текущая галерея намеренно переводит последний элемент первого набора в длинное
  полотно отзывов; плавного drag нет, смена происходит после `pointerup`. Это
  подтверждает описанный владельцем дефект, а не случайный браузерный сбой.
- Голосовые уже находятся в каноническом HTML внутри отключённого template.
  Нужна активация и один renderer, а не новый независимый виджет.
- Общий shell уже имеет `site-header.js`, `site-footer.js` и
  `site-cookie-notice.js`, но ownership footer/header/consent расходится между
  записями. До кода его нужно свести к одному platform-owned контракту.
- Главные T123-риски: повторное выполнение scripts после монтажа, дубли
  header/footer/counter, неполное переписывание относительных URL и расхождение
  server preview с автономным production artifact.

Репрезентативные локальные проверки: карточка краткой программы;
gallery/lightbox; голосовой отзыв; общая sticky-панель popup; плавающий VSL;
увеличение документов. В user-spec они сведены в четыре согласования, чтобы не
заставлять владельца принимать каждую мелочь отдельно.
