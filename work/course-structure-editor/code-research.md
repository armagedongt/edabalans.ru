...
После исследования владелец запретил добавление, удаление и перестановку материалов
в редакторе. Кнопки этих операций, миграция прогресса материалов на `step_id` и
соответствующие journey не входят в первую реализацию. Материалы сохраняют позиции;
скрытие не удаляет элемент из массива и не сдвигает соседние индексы. Изменение
состава и порядка выполняется только отдельной задачей в чате.
