...
Итог интервью: отдельный public/admin override не входит в функцию. При сбое
обоих ботов обязательный шаг остаётся незавершённым, а поддержка использует уже
существующее управление данными/прогрессом. Explicit send не создаёт отдельную
долгоживущую pending-сущность: обычный deep-link token содержит intent, и его
consume одновременно связывает account и идемпотентно ставит материал в отправку;
без Start или после expiry ничего не происходит.
