...
function showOffers(data){screen.innerHTML=`<main class="mc mc-offer-page"><h1>Можно добавить к мастер-классу</h1><p class="lead">Выберите дополнительные материалы, которые помогут продолжить работу после мастер-класса.</p>${window.EdabalansMasterclassOfferView.markup(data)}</main>`;bindPreview(data);startTimer(data.expires_at)}
