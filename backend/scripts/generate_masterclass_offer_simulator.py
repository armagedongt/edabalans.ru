...
from app.masterclass_offer_catalog import (  # noqa: E402
...
<div class="sim-shell">
  <section class="sim-controls" aria-label="Конструктор сценария">
    ...
    <div class="sim-owned"><div class="sim-owned-title">Что уже куплено к этому моменту</div>__OWNED_CONTROLS__</div>
...
async function render(){const token=++renderToken,cfg=stages[placement.value],remaining=cfg.hours==null?null:Math.max(0,cfg.hours-(cfg.elapsedHours||0)),duration=cfg.hours?`${cfg.hours} ч`:'без таймера',windowAction=cfg.hours?(cfg.starts?'запускается при первом открытии':`продолжается без перезапуска${cfg.elapsedHours?`; в превью показано, что прошло ${cfg.elapsedHours} ч`:''}`):'не запускается';facts.innerHTML=`<strong>Системная точка:</strong> ${cfg.placement} · <strong>событие:</strong> ${cfg.event} · <strong>ступень:</strong> ${cfg.stage} · <strong>окно:</strong> ${duration}, ${windowAction}. Карточки рассчитывает тот же backend, что и для клиента.`;const response=await fetch('/api/masterclass/admin/offer-preview',{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:JSON.stringify({stage_code:cfg.stage,placement:cfg.placement,owned_product_codes:owned(),tariff_name:tariffs[tariff.value].name,remaining_hours:remaining})});if(!response.ok)throw new Error('preview unavailable');const data=await response.json();if(token!==renderToken)return;screen.innerHTML=`<main class="mc mc-offer-page"><h1>Можно добавить к мастер-классу</h1><p class="lead">Выберите дополнительные материалы, которые помогут продолжить работу после мастер-класса.</p>${window.EdabalansMasterclassOfferView.markup(data)}</main>`;screen.querySelectorAll('[data-offer]').forEach(button=>button.onclick=()=>{const card=data.offers.find(item=>item.code===button.dataset.offer);(card?.items||[]).forEach(code=>{const check=checks.find(item=>item.dataset.owned===code);if(check)check.checked=true});render()});startTimer(data.expires_at)}
function renderPriceMap(rows,presentation){const order=['early','second','review','last_week','standard'],labels={early:'1–2',second:'3',review:'4',last_week:'5',standard:'6'},byCode=Object.fromEntries(rows.map(row=>[row.code,row])),short=presentation==='site_short_v1';const cells=['Цена',...order.map(code=>labels[code])],price=code=>byCode[code]?.pricing||{},moneyValue=value=>value==null?'—':new Intl.NumberFormat('ru-RU').format(value);const rules=short?[['Рецепты',code=>price(code).single],['Рецепты + калорийный',code=>price(code).bundle?.['2']],['Тариф с консультацией',code=>{const current=price(code),addon=current.site_short?.consultation_addon;return addon==null?null:Number(current.bundle?.['2']||0)+Number(addon)}],['Консультация отдельно',code=>['review','standard'].includes(code)?price(code).consultation:null]]:[['1 продукт',code=>price(code).single],['2 продукта',code=>price(code).bundle?.['2']],['3 продукта',code=>price(code).bundle?.['3']],['4 продукта',code=>price(code).bundle?.['4']],['Консультация',code=>price(code).consultation]];rules.forEach(([label,get])=>cells.push(label,...order.map(code=>moneyValue(get(code))));priceGrid.innerHTML=cells.map(value=>`<span>${value}</span>`).join('')}
async function loadCanonicalData(){try{const stageResponse=await fetch('/api/masterclass/admin/offer-stages',{credentials:'same-origin'});if(!stageResponse.ok)throw new Error('offer stage catalog unavailable');const stagePayload=await stageResponse.json(),byCode=Object.fromEntries(stagePayload.stages.map(item=>[item.code,item]));Object.values(stages).forEach(cfg=>{const row=byCode[cfg.stage];if(!row)throw new Error(`missing stage ${cfg.stage}`);if(cfg.hours!=null&&row.duration_hours!=null)cfg.hours=Number(row.duration_hours)});renderPriceMap(stagePayload.stages,stagePayload.presentation);const pricingResponse=await fetch('/admin/api/pricing',{credentials:'same-origin'});if(pricingResponse.ok){const pricingPayload=await pricingResponse.json(),version=pricingPayload.versions.find(item=>item.status==='active')||pricingPayload.versions[0];if(version){const tariffMap={'site.masterclass.basic':'minimal','site.masterclass.recipes':'standard','site.masterclass.consult':'consult'},resourceMap={ACCESS_RECIPES:'recipes',ACCESS_CONSULTATION:'consultation'};version.entries.filter(entry=>tariffMap[entry.code]&&entry.enabled).forEach(entry=>{const key=tariffMap[entry.code],item=tariffs[key];item.name=entry.name;item.owned=(entry.resource_codes||[]).map(code=>resourceMap[code]).filter(Boolean);tariff.querySelector(`option[value="${key}"]`).textContent=`${entry.name} · ${money(entry.sale_amount)}`})}}await render()}catch(error){facts.textContent='Не удалось загрузить действующий каталог предложений. Предпросмотр цен отключён, чтобы не показывать устаревшие данные.';screen.innerHTML='<main class="mc mc-offer-page"><section class="mc-card mc-offer-empty"><h2>Предпросмотр временно недоступен</h2><p>Обновите страницу или проверьте серверный каталог предложений.</p></section></main>';priceGrid.innerHTML=''}}
...
def render_simulator() -> str:
    ...
    visible_product_codes = (
        SITE_SHORT_OFFER_PRESENTATION["digital_product_codes"] + ("consultation",)
        if ACTIVE_OFFER_PRESENTATION == SITE_SHORT_OFFER_PRESENTATION["code"]
        else tuple(OFFER_PRODUCTS)
    )
    owned_controls = "".join(
        f'<label><input type="checkbox" data-owned="{code}"> '
        f'{OFFER_PRODUCTS[code]["name"]}</label>'
        for code in visible_product_codes
    )
    return (
        ...
        .replace("__OWNED_CONTROLS__", owned_controls)
    )
