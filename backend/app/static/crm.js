(function () {
  ...
  async function renderUsers(buyersOnly) {
    ...
    root.innerHTML = top("users") + `
      ...
      <div class="crm-quick-filters" aria-label="Быстрые фильтры"><button class="crm-chip ${filters.buyer_kind === "all" && filters.masterclass_access === "" ? "active" : ""}" data-people-all>Все</button><button class="crm-chip ${filters.buyer_kind === "buyers" ? "active" : ""}" data-buyer-kind="buyers">Покупатели</button><button class="crm-chip ${filters.buyer_kind === "non_buyers" ? "active" : ""}" data-buyer-kind="non_buyers">Без покупок</button><button class="crm-chip ${filters.masterclass_access === "true" ? "active" : ""}" data-masterclass-access="true">Есть МК</button><button class="crm-chip ${filters.masterclass_access === "false" ? "active" : ""}" data-masterclass-access="false">Нет МК</button><button class="crm-chip" data-view="access">Проблемы доступа</button></div>
    ...
    root.querySelector("[data-people-all]").addEventListener("click", () => { state.userFilters.buyer_kind = "all"; state.userFilters.masterclass_access = ""; state.offset = 0; renderUsers(false).catch(showError); });
