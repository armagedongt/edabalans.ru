...
class MarketingDailyReport(Base):
    """Daily marketing snapshot used by Telegram delivery and later analysis."""

    __tablename__ = "marketing_daily_reports"

    id: Mapped[uuid.UUID] = uuid_pk()
    report_date: Mapped[str] = mapped_column(String(10), unique=True, nullable=False)
    period_start: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
    period_end: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
    payload_json: Mapped[dict] = mapped_column(JSON, default=dict, nullable=False)
    telegram_messages: Mapped[list] = mapped_column(JSON, default=list, nullable=False)
...
class CourseStageProgress(Base):
    ...
    timezone_name: Mapped[str] = mapped_column(
        String(64), default="Europe/Moscow", server_default=text("'Europe/Moscow'"), nullable=False
    )
    task_opened_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    structure_revision_no: Mapped[int] = mapped_column(
        Integer, default=1, server_default=text("1"), nullable=False
